...
# divergence_log CSV 列 (固定順、段階 3 で age_factor / 観察版 L2 を追加)
# 段階 2 の theta_diff_norm は theta_diff_norm_all に改名 (計算は同じ)
...
def write_divergence_log_csv(
    ...
    """
    registry 全 cid の divergence_log を 1 ファイルに集約 (段階 3)。
    cid_id 昇順、cid 内は step 昇順。

    段階 3 新規列: age_factor, n_observed, theta_diff_norm_observed,
                   theta_diff_norm_observed_normalized。
    段階 2 の theta_diff_norm は theta_diff_norm_all に改名 (計算は同じ)。
    """
    rows: list[dict] = []
    for cid in sorted(registry.keys()):
        ...
        for entry in log:
            # 段階 1/2 互換: theta_diff_norm のみ持つ古いエントリも読めるように
            theta_diff_all = entry.get(
                "theta_diff_norm_all",
                entry.get("theta_diff_norm", 0.0),
            )
            rows.append({
                "seed": seed,
                ...
                "age_factor": round(float(entry.get("age_factor", 0.0)), 6),
                "n_observed": int(entry.get("n_observed", 0)),
                "theta_diff_norm_all": round(float(theta_diff_all), 6),
                "theta_diff_norm_observed": round(
                    float(entry.get("theta_diff_norm_observed", 0.0)), 6),
                "theta_diff_norm_observed_normalized": round(
                    float(entry.get(
                        "theta_diff_norm_observed_normalized", 0.0)), 6),
