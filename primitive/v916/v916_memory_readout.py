...
(B を read-only でのみ参照)。v916_cid_self_buffer.py は [B]、
v916_a_observer.py / v916_divergence_tracker.py は [A]。

出力ディレクトリ: diag_v916_{tag}/
...
USAGE:
  # smoke
  python v916_memory_readout.py --seed 0 --maturation-windows 5 \\
      --tracking-windows 2 --window-steps 100 --tag smoke

  # 本番 (24 seeds)
  seq 0 23 | parallel -j24 python v916_memory_readout.py \\
...
# v9.15: Layer C (CID self-buffer, additive-only, read-only observation)
from v916_fetch_operations import ensure_cid_registered as v915_ensure_cid_registered
# v9.15 段階 2: 50 step 駆動の pulse_fetch は廃止 (event 駆動 Fetch に置換)。
# pulse_fetch 関数自体は v916_fetch_operations.py に残置 (将来の Paired Audit 用)。
# v9.15: A-side observer (read-only view of CidSelfBuffer)
from v916_a_observer import (
    build_v915_subject_columns as v915_build_subject_columns,
    write_per_cid_self_csv as v915_write_per_cid_self_csv,
    write_observation_log_csv as v916_write_observation_log_csv,
)
# v9.15: Self-Divergence Tracker (A-side, read-only)
from v916_divergence_tracker import (
...
def v916_q0_from_cog(cog, cid) -> int:
...
def run(seed=42, maturation_windows=20, tracking_windows=10,
    ...
    for w in range(maturation_windows):
        ...
        for lid in (curr_lids - prev_lids):
            ...
            v915_ensure_cid_registered(
                ...
                Q0=v916_q0_from_cog(cog, cog.cid_of_lid.get(lid)))
    ...
    for tw in range(tracking_windows):
        ...
        for step in range(window_steps):
            ...
            #                              touch しない)
            for v915_ev in v914_ledger.events[v915_ev_count_before:]:
                v915_buf = v915_self_buffers.get(v915_ev.get("cid"))
                if v915_buf is None:
                    continue
        ...
        for lid in (curr_lids - prev_lids):
            ...
            v915_ensure_cid_registered(
                ...
                lab["nodes"], engine.state, v913_global_step,
                Q0=v916_q0_from_cog(cog, new_cid))
    ...
    # ════════════════════════════════════════════════════════
    v916_obslog_path = outdir / "selfread" / f"observation_log_seed{seed}.csv"
    v916_n_obslog = v916_write_observation_log_csv(
        v915_self_buffers, seed, v916_obslog_path)
    print(f"  v9.16 Layer C: observation_log: {v916_n_obslog} rows "
          f"-> {v916_obslog_path}")
