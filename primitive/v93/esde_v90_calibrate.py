#!/usr/bin/env python3
"""
ESDE v9.0 — Self-Referential Feedback Loop (Phase 1)
======================================================
v8.5 local wave + label tracking + Stage 1 thaw
+ v9 global feedback: died_share_sum → EMA → torque modulation.

Changes from v8.5:
  - VirtualLayer import changed to virtual_layer_v9
  - CSV: 6 new columns for feedback stats
  - Engine/genesis_physics: ZERO changes

USAGE
-----
  python esde_v90_calibrate.py --seed 42 --windows 200 --local-amp 0.3
"""
...
# v9: Import feedback-enabled VirtualLayer
...
LOG_FIELDS = [
    ...
    "v_torque_n", "v_torque_contested", "v_mean_torque", "v_top_share",
    "v_label_rplus_rate",
    ...
    "thaw_mean_range",
    # v9.0 feedback loop
    "fb_died_share", "fb_ema", "fb_ratio", "fb_multiplier", "fb_warmup",
    # v9.3 deviation detection
    "dev_mean_score", "dev_max_score", "dev_n_responding", "dev_mean_gf",
]
...
def run(seed, n_windows, window_steps, output_dir, encap_params, N,
    ...
    # ── v9: Replace VirtualLayer with feedback-enabled version ──
    ...
    tag = f"v90_N{N}_seed{seed}_{tag_str.replace('+','_')}"
    ...
    for w in range(n_windows):
        ...
        if local_amplitude > 0:
            ...
            if frame.window == 50 and not rep_labels:
                ...
                print(f"  [v9.0] Selected {len(rep_labels)} representative labels"
                      f" at w{frame.window}")
        ...
        row = {
            ...
            "v_torque_contested": vl.get("torque_contested", 0),
            ...
            # v9.0 feedback loop
            "fb_died_share": vl.get("died_share_sum", 0),
            "fb_ema": vl.get("turnover_ema", 0),
            "fb_ratio": vl.get("signal_ratio", 0),
            "fb_multiplier": vl.get("torque_multiplier", 1.0),
            "fb_warmup": vl.get("warmup_active", 1),
            # v9.3 deviation
            "dev_mean_score": vl.get("dev_mean_score", 0),
            "dev_max_score": vl.get("dev_max_score", 0),
            "dev_n_responding": vl.get("dev_n_responding", 0),
            "dev_mean_gf": vl.get("dev_mean_gf", 1.0),
        }
        writer.writerow(row)
        f.flush()
    ...
    if local_amplitude > 0:
        ...
        # v9.3: deviation detection log
        if hasattr(engine.virtual, '_deviation_log') and engine.virtual._deviation_log:
            detail["deviation_log"] = engine.virtual._deviation_log
...
def main():
    parser = argparse.ArgumentParser(
        description="ESDE v9.0 Self-Referential Feedback Loop")
    ...
    parser.add_argument("--output", type=str, default="calibration_v90")
    ...
    parser.add_argument("--gamma", type=float, default=0.10,
                        help="Feedback gamma (sensitivity). Negative = destabilizing")
    parser.add_argument("--clamp-lo", type=float, default=0.8,
