...
(B を read-only でのみ参照)。v915s2_cid_self_buffer.py は [B]、
v915s2_a_observer.py / v915s2_divergence_tracker.py は [A]。

出力ディレクトリ: diag_v915s2_{tag}/
...
USAGE:
  # smoke
  python v915s2_memory_readout.py --seed 0 --maturation-windows 5 \\
      --tracking-windows 2 --window-steps 100 --tag smoke

  # 本番 (24 seeds)
  seq 0 23 | parallel -j24 python v915s2_memory_readout.py \\
...
from v915s2_fetch_operations import ensure_cid_registered as v915_ensure_cid_registered
# v9.15 段階 2: 50 step 駆動の pulse_fetch は廃止 (event 駆動 Fetch に置換)。
# pulse_fetch 関数自体は v915s2_fetch_operations.py に残置 (将来の Paired Audit 用)。
# v9.15: A-side observer (read-only view of CidSelfBuffer)
from v915s2_a_observer import (
...
from v915s2_divergence_tracker import (
...
def run(seed=42, maturation_windows=20, tracking_windows=10,
    ...
    outdir = Path(f"diag_v915s2_{tag}")
    ...
    for tw in range(tracking_windows):
        ...
        for step in range(window_steps):
            ...
            # v9.15 段階 2: event 駆動 Fetch (Layer B spend → Layer C fetch の順)
