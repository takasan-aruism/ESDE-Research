...
def run(seed=42, maturation_windows=20, tracking_windows=10,
    ...
    outdir = Path(f"diag_v97_feedback_{tag}")
    ...
    for tw in range(tracking_windows):
        ...
        # ── v9.7 Cognitive feedback: z-score based torque modulation ──
        # Population-derived (no fixed parameters).
        # tanh(z_social) maps to [-1, +1] → torque_factor in [0, 2]
        torque_factors_log = {}
        if len(socials) >= 2:
            mean_s = np.mean(socials)
            std_s = np.std(socials)
            for idx, lid in enumerate(alive_tracked):
                if std_s > EPS:
                    z = (socials[idx] - mean_s) / std_s
                    tf = 1.0 + math.tanh(z)
                else:
                    tf = 1.0
                vl.labels[lid]["torque_factor"] = tf
                torque_factors_log[lid] = tf
        else:
            for lid in alive_tracked:
                vl.labels[lid]["torque_factor"] = 1.0
                torque_factors_log[lid] = 1.0
        ...
        window_rows.append({
            ...
            "tf_mean": round(np.mean(list(torque_factors_log.values())), 4) if torque_factors_log else 1.0,
            "tf_std": round(np.std(list(torque_factors_log.values())), 4) if torque_factors_log else 0.0,
            "tf_max": round(max(torque_factors_log.values()), 4) if torque_factors_log else 1.0,
            "tf_min": round(min(torque_factors_log.values()), 4) if torque_factors_log else 1.0,
        })
