#!/usr/bin/env python3
"""
ESDE v9.13 — Persistence-based Label Birth (builds on v9.11 Cognitive Capture)
===============================================================================
v9.11 (primitive/v911/v911_cognitive_capture.py) をベースに label birth 判定を改変:
  - S>=0.20 island 検出 (経路A) を age_r>=tau の connected component に置換
  - R>0 ペア経路 (経路B) を廃止 (stash 方式で無効化)
  - age_r (連続 R>0 step 数) カウンタを Resonance 直後に常時更新
  - per_subject CSV に v913_ prefix 列を追加

物理層 (v19g_canon, esde_v82_engine), VL 本体 (virtual_layer_v9), 認知層 capture には介入しない。

USAGE:
  # τ=50
  seq 0 23 | parallel -j24 python v913_persistence_birth.py \\
      --seed {} --tau 50 --tracking-windows 20 --tag main

  # τ=100
  seq 0 23 | parallel -j24 python v913_persistence_birth.py \\
      --seed {} --tau 100 --tracking-windows 20 --tag main
...
_V911_DIR = _REPO_ROOT / "primitive" / "v911"   # v9.13: for v911_genesis_budget_measure

for p in [str(_SCRIPT_DIR), str(_V82_DIR), str(_V43_DIR),
          str(_V41_DIR), str(_ENGINE_DIR), str(_V910_DIR), str(_V911_DIR)]:
...
# ═══════════════════════════════════════════════════════════════════════
# v9.13: Persistence-based Label Birth
# ═══════════════════════════════════════════════════════════════════════
# R>0 連続持続 step 数 (age_r) の閾値。この値以上持続したリンクのみを
# 連結成分抽出に使う。Step 0 audit 分布から Taka が決定。
# 本番は τ=50 と τ=100 の 2 条件を並列実行する (run 時 --tau で切替)。
V913_TAU_DEFAULT = 50  # CLI で --tau で上書き可
...
def build_connected_components(edges):
    """v9.13: edges (list of (node_i, node_j)) を Union-Find で連結成分化。
    Returns: list of frozenset(nodes), size >= 2 のみ。
    """
    parent = {}
    def find(x):
    ...
    return [frozenset(nodes) for nodes in comps.values() if len(nodes) >= 2]
...
# ─────────────────────────────────────────────────────────────────
# Information pickup parameters (v9.8c)
# ─────────────────────────────────────────────────────────────────
# PROVISIONAL ENGINEERING VALUES.
# These are NOT theoretical truths. They are the initial probe settings.
# Subject to revision after v9.8c output review.
#
# GPT 監査 §9 の重要な注意:
#   INFORMATION_LIFETIME は最も sensitivity が高いパラメータ。
#   結果を見て暗黙に再解釈せず、measure first.
#
# Aruism 整合:
#   - これらの値は物理層には触れない (TTL bonus は認知層内のみ)
#   - 競争ルール (winner = closest phase) は構造的選択であって、
#     特定 cid を優遇しない
INFORMATION_LIFETIME = 3       # 死亡情報がプールに残る window 数
                               # Taka: "ESDE には軽めで良い"
AFFINITY_THRESHOLD = math.pi / 8  # 拾得親和性の phase 距離閾値 (約 22.5°)
TTL_BONUS_PER_PICKUP = 1       # 1 拾得あたりの TTL 延長量 (window)
...
class SubjectLayer:
    ...
    def __init__(self):
        ...
        # ──────────────────────────────────────────────
        # v9.8c: Information pickup state
        # ──────────────────────────────────────────────
        # death_pool: グローバルな死亡情報プール (空間構造なし)
        #   各エントリ: {record_id, dead_lid, phase_sig, deposited_at, status}
        #   status: "active" / "consumed" / "expired"
        # cid_ttl_bonus: 各 cid の累積 TTL bonus
        #   reap_ghosts での effective_ttl = GHOST_TTL + bonus
        #   hosted の間に蓄積される。ghost 化後は変わらない。
        self.death_pool = []
        self.cid_ttl_bonus = {}
        self._next_record_id = 0

        # 統計用
        self._reaped_history = []  # [{cid, born, host_lost, reaped, ghost_dur, effective_ttl}]
        self._tag_history = []     # [{cid, window, prev, current, delta, tags, state}]
        self._pickup_log = []      # v9.8c: [{record_id, picker_cid, losers, ...}]
        self._death_pool_log = []  # v9.8c: [{record_id, dead_lid, deposited, removed, reason}]
    ...
    def birth(self, lid, phase_sig, current_window):
        ...
        # v9.8c: pickup state
        # cid_ttl_bonus は新生時 0、拾得で蓄積される
        self.cid_ttl_bonus[cid] = 0
    ...
    def reap_ghosts(self, current_window):
        ...
        v9.8c: effective_ttl = GHOST_TTL + cid_ttl_bonus[cid]
        ...
        for cid in list(self.host_lost_at.keys()):
            ...
            if lost_w is None:
                continue  # まだ host 中
            # v9.8c: effective TTL with bonus
            ttl_bonus = self.cid_ttl_bonus.get(cid, 0)
            effective_ttl = GHOST_TTL + ttl_bonus
            if current_window - lost_w >= effective_ttl:
                # 完全削除
                ...
                self._reaped_history.append({
                    ...
                    "ttl_bonus": ttl_bonus,            # v9.8c
                    "effective_ttl": effective_ttl,    # v9.8c
                })
                ...
                # v9.8c: clean up pickup state
                self.cid_ttl_bonus.pop(cid, None)
    ...
    # ──────────────────────────────────────────────────────
    def record_death(self, dead_lid, phase_sig, current_window):
        """label が cull された時に呼ぶ。
        死亡情報プールにスナップショットを追加する。
        グローバルプール (空間構造なし)。
        Aruism 整合: 物理層には触れない、認知層内のメタデータ追加のみ。
        """
        ...
        record = {
            "record_id": record_id,
            "dead_lid": dead_lid,
            "phase_sig": float(phase_sig),
            "deposited_at": current_window,
            "status": "active",
        }
        self.death_pool.append(record)
        # 出生ログ
        self._death_pool_log.append({
            "record_id": record_id,
            "dead_lid": dead_lid,
            "phase_sig": round(float(phase_sig), 4),
            "deposited_at": current_window,
            "removed_at": None,
            "removal_reason": None,
        })
    ...
    def cleanup_death_pool(self, current_window):
        """LIFETIME 切れの record を削除する。
        毎 window 末に呼ぶ。

        セマンティクス (#V98C-1 修正済み):
          age = current_window - deposited_at
          age >= INFORMATION_LIFETIME → expired
          → LIFETIME=3 の場合、age=0,1,2 がアクティブ (3 windows)
          → 「死亡情報がプールに残る window 数 = LIFETIME」に統一
        """
        new_pool = []
        for record in self.death_pool:
            age = current_window - record["deposited_at"]
            if age >= INFORMATION_LIFETIME:
                # 期限切れ削除
                self._mark_record_removed(record["record_id"],
                                          current_window, "expired")
            else:
                new_pool.append(record)
        self.death_pool = new_pool

    def _mark_record_removed(self, record_id, current_window, reason):
        """death_pool_log の record を終了状態に更新。"""
        for entry in self._death_pool_log:
            if entry["record_id"] == record_id and entry["removed_at"] is None:
                entry["removed_at"] = current_window
                entry["removal_reason"] = reason
                return

    def attempt_pickup_round(self, current_window):
        """全 hosted cid と全 active death_record の間で、
        排他的競争による拾得を 1 ラウンドだけ行う。

        ルール:
          1. 各 record について、AFFINITY_THRESHOLD を満たす hosted cid の集合を求める
          2. 候補が 0 → record はそのまま (LIFETIME 切れまで残る)
          3. 候補が 1 → そのまま勝者 (uncontested)
          4. 候補が 2+ → phase 距離が最も近い cid が勝者 (contested)
          5. 勝者の cid_ttl_bonus に TTL_BONUS_PER_PICKUP を加算
          6. record を consume (death_pool から削除、log を更新)
          7. 全候補 (勝者と敗者) を pickup_log に記録

        Returns: (n_uncontested, n_contested, n_no_eligible)
        """
        # Snapshot of hosted cids and their phases
        hosted_phases = {}
        for cid in list(self.current_lid.keys()):
            if self.is_hosted(cid) and cid in self.original_phase_sig:
                hosted_phases[cid] = self.original_phase_sig[cid]
        ...
        for record in self.death_pool:
            ...
            # Find eligible cids (within AFFINITY_THRESHOLD)
            record_phase = record["phase_sig"]
            candidates = []  # [(cid, phase_dist)]
            for cid, cid_phase in hosted_phases.items():
                phase_dist = _circular_diff(cid_phase, record_phase)
                if abs(phase_dist) <= AFFINITY_THRESHOLD:
                    candidates.append((cid, abs(phase_dist)))

            if not candidates:
                n_no_eligible += 1
                continue

            # Determine winner: smallest phase distance
            candidates.sort(key=lambda x: x[1])
            winner_cid, winner_dist = candidates[0]
            losers = [(c, d) for c, d in candidates[1:]]
            ...
            # Apply TTL bonus to winner.
            # NOTE: winner_cid is guaranteed to be hosted because hosted_phases
            # was built from is_hosted() filter at L621-624. Ghost cids never
            # enter the candidate pool. This is the design contract from GPT
            # audit §6 (Hosted-only Rule).
            self.cid_ttl_bonus[winner_cid] = (
                self.cid_ttl_bonus.get(winner_cid, 0) + TTL_BONUS_PER_PICKUP
            )

            # Mark record as consumed
            record["status"] = "consumed"
            consumed_record_ids.append(record["record_id"])
            self._mark_record_removed(record["record_id"], current_window, "consumed")

            # Log the pickup event
            self._pickup_log.append({
                "record_id": record["record_id"],
                "dead_lid": record["dead_lid"],
                "window": current_window,
                "winner_cid": winner_cid,
                "winner_phase_dist": round(winner_dist, 6),
                "winner_new_ttl_bonus": self.cid_ttl_bonus[winner_cid],
                "n_candidates": len(candidates),
                "outcome": outcome,
                "loser_cids": "|".join(str(c) for c, _ in losers) if losers else "",
                "loser_phase_dists": "|".join(f"{d:.4f}" for _, d in losers) if losers else "",
            })

        # Remove consumed records from death_pool
        self.death_pool = [r for r in self.death_pool if r["status"] == "active"]

        return n_uncontested, n_contested, n_no_eligible
...
def run(seed=42, maturation_windows=20, tracking_windows=10,
        window_steps=500, tag="short", tau=V913_TAU_DEFAULT):

    t_start = time.time()
    N = V82_N
    torus_sub = build_torus_substrate(N)

    outdir = Path(f"diag_v913_persistence_tau{tau}_{tag}")
    ...
    # v9.13: age_r (連続 R>0 step 数) カウンタ
    age_r = {}
    max_age_r = {}
    # v9.13: label birth 時のメンバーリンク情報 (per_subject v913_ 列用)
    v913_birth_member_info = {}  # cid -> {"age_r_min", "age_r_mean", "max_age_r_mean", "n_edges"}
    ...
    for tw in range(tracking_windows):
        ...
        for step in range(window_steps):
            ...
            # v9.13: age_r 更新 (Resonance 直後、Auto-Growth より前)
            for k in engine.state.alive_l:
                if engine.state.R.get(k, 0.0) > 0:
                    age_r[k] = age_r.get(k, 0) + 1
                else:
                    age_r[k] = 0
                if age_r[k] > max_age_r.get(k, 0):
                    max_age_r[k] = age_r[k]
        ...
        window_rows.append({
            ...
            # v9.8c 新規列 (情報拾得統計, 後で update する)
            "ghost_births": 0,
            "ghost_reaped": 0,
            "pickup_uncontested": 0,
            "pickup_contested": 0,
            "pickup_no_eligible": 0,
            "pickup_total_success": 0,
            "death_pool_size": 0,
            "mean_ttl_bonus": 0.0,
        })
        ...
        # v9.13: persistence-based islands (S>=0.20 置換)
        #   旧: find_islands_sets(engine.state, 0.20)
        #   新: age_r >= tau の edge で connected components
        persistent_edges = [k for k in engine.state.alive_l
                            if age_r.get(k, 0) >= tau]
        islands_v913 = build_connected_components(persistent_edges)

        class _Isl:
            pass
        islands_dict = {}
        for i, node_set in enumerate(islands_v913):
            obj = _Isl()
            obj.nodes = node_set
            islands_dict[i] = obj

        # v9.13: R>0 ペア経路を封じるため R を一時的に stash
        stash_R = dict(engine.state.R)
        for k in list(engine.state.R.keys()):
            engine.state.R[k] = 0.0

        vs = vl.step(engine.state, engine.window_count,
                      islands=islands_dict, substrate=engine.substrate)

        # v9.13: R を復元
        engine.state.R = stash_R
        ...
        for lid in (curr_lids - prev_lids):
            ...
            # v9.13: メンバーリンクの persistence 情報を記録
            v913_label_nodes = set(lab["nodes"])
            v913_member_links = [k for k in engine.state.alive_l
                                 if k[0] in v913_label_nodes and k[1] in v913_label_nodes]
            if v913_member_links:
                v913_ars = [age_r.get(k, 0) for k in v913_member_links]
                v913_mars = [max_age_r.get(k, 0) for k in v913_member_links]
                v913_birth_member_info[new_cid] = {
                    "age_r_min": min(v913_ars),
                    "age_r_mean": round(sum(v913_ars) / len(v913_ars), 2),
                    "max_age_r_mean": round(sum(v913_mars) / len(v913_mars), 2),
                    "n_edges": len(v913_member_links),
                }
            else:
                v913_birth_member_info[new_cid] = {
                    "age_r_min": 0, "age_r_mean": 0,
                    "max_age_r_mean": 0, "n_edges": 0,
                }
        ...
        for lid in (prev_lids - curr_lids):
            ...
            if lid in label_meta:
                ...
                all_deaths.append({"lid": lid,
                                   "cid": cid, "w": w})
                # v9.8c: record death in death_pool
                # phase_sig は label_meta から取る (vl.labels から既に消えている)
                dead_phase_sig = label_meta[lid].get("phase_sig", 0.0)
                cog.record_death(lid, dead_phase_sig, w)
            if cid is not None and cid in subject_meta:
                subject_meta[cid]["host_lost_window"] = w
                subject_meta[cid]["final_state"] = "ghost"

        # v9.8c: Pickup round (hosted cid が排他的競争で record を拾う)
        # 順序: detach → record_death → attempt_pickup → cleanup → reap
        # この順序により、この window で死んだ label もこの window で拾える
        n_uncon, n_con, n_none = cog.attempt_pickup_round(w)
        cog.cleanup_death_pool(w)

        # Reap ghosts (TTL, v9.8c では effective_ttl)
        ...
        # Update window row with reap count + v9.8c pickup stats
        window_rows[-1]["ghost_births"] = ghost_births_this_window
        window_rows[-1]["ghost_reaped"] = len(reaped_cids)
        window_rows[-1]["pickup_uncontested"] = n_uncon
        window_rows[-1]["pickup_contested"] = n_con
        window_rows[-1]["pickup_no_eligible"] = n_none
        window_rows[-1]["pickup_total_success"] = n_uncon + n_con
        window_rows[-1]["death_pool_size"] = len(cog.death_pool)
        # 全 hosted cid の bonus 平均 (この時点でのスナップショット)
        bonuses = [cog.cid_ttl_bonus.get(c, 0)
                   for c in cog.all_hosted_cids()]
        window_rows[-1]["mean_ttl_bonus"] = (
            float(np.mean(bonuses)) if bonuses else 0.0
        )
    ...
    # v9.8c: 各 cid の pickup 統計を集計
    # cid ごとの (勝った数、負けた数) を pickup_log から数える
    cid_pickups_won = Counter()
    cid_pickups_lost = Counter()
    for log_entry in cog._pickup_log:
        cid_pickups_won[log_entry["winner_cid"]] += 1
        loser_str = log_entry.get("loser_cids", "")
        if loser_str:
            for loser_id_str in loser_str.split("|"):
                try:
                    cid_pickups_lost[int(loser_id_str)] += 1
                except ValueError:
                    pass

    # cid ごとの最終 ttl_bonus (reaped なら _reaped_history から、
    # まだ生きてるなら cog.cid_ttl_bonus から取る)
    final_ttl_bonus = {}
    for r in cog._reaped_history:
        final_ttl_bonus[r["cid"]] = r["ttl_bonus"]
    for cid, b in cog.cid_ttl_bonus.items():
        final_ttl_bonus[cid] = b
    ...
    for cid, meta in subject_meta.items():
        ...
        # v9.8c: pickup 統計
        ttl_bonus = final_ttl_bonus.get(cid, 0)
        n_pickups_won = cid_pickups_won.get(cid, 0)
        n_pickups_lost = cid_pickups_lost.get(cid, 0)
        effective_ttl = GHOST_TTL + ttl_bonus
        ...
        subject_rows.append({
            ...
            # v9.8c: pickup columns
            "ttl_bonus": ttl_bonus,
            "n_pickups_won": n_pickups_won,
            "n_pickups_lost": n_pickups_lost,
            ...
            **(lambda _bg=cog.v11_b_gen.get(cid),
            ...
            # v9.13: persistence birth 情報
            "v913_tau": tau,
            "v913_birth_age_r_min": v913_birth_member_info.get(cid, {}).get("age_r_min", ""),
            "v913_birth_age_r_mean": v913_birth_member_info.get(cid, {}).get("age_r_mean", ""),
            "v913_birth_max_age_r_mean": v913_birth_member_info.get(cid, {}).get("max_age_r_mean", ""),
            "v913_n_member_edges": v913_birth_member_info.get(cid, {}).get("n_edges", ""),
        })
    ...
    # ════════════════════════════════════════════════════════
    # SAVE: Pickup log (v9.8c 新規)
    # ════════════════════════════════════════════════════════
    # 全拾得イベント (uncontested + contested)。勝者と敗者を完全記録。
    if cog._pickup_log:
        pickup_path = outdir / "pickup" / f"pickup_log_seed{seed}.csv"
        with open(pickup_path, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=cog._pickup_log[0].keys())
            writer.writeheader()
            writer.writerows(cog._pickup_log)

    # ════════════════════════════════════════════════════════
    # SAVE: Death pool log (v9.8c 新規)
    # ════════════════════════════════════════════════════════
    # 全 death record の出生〜消滅。removal_reason: consumed / expired
    if cog._death_pool_log:
        dpool_path = outdir / "pickup" / f"death_pool_log_seed{seed}.csv"
        with open(dpool_path, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=cog._death_pool_log[0].keys())
            writer.writeheader()
            writer.writerows(cog._death_pool_log)
    ...
    n_records_total = len(cog._death_pool_log)
    n_records_consumed = sum(1 for e in cog._death_pool_log
                             if e.get("removal_reason") == "consumed")
    n_records_expired = sum(1 for e in cog._death_pool_log
                            if e.get("removal_reason") == "expired")
    n_records_active = sum(1 for e in cog._death_pool_log
                           if e.get("removed_at") is None)

    n_pickup_uncontested = sum(1 for e in cog._pickup_log
                                if e.get("outcome") == "uncontested")
    n_pickup_contested = sum(1 for e in cog._pickup_log
                              if e.get("outcome") == "contested")

    # bonus 統計
    all_bonuses = []
    for r in cog._reaped_history:
        all_bonuses.append(r.get("ttl_bonus", 0))
    for cid, b in cog.cid_ttl_bonus.items():
    ...
    if all_tags:
        ...
        for tag in ["gain_social", "loss_social",
            ...
            print(f"      {tag:>18}: {n}")
    # v9.8c: Pickup statistics
    print(f"    --- v9.8c Pickup ---")
    print(f"    death records:    {n_records_total}")
    print(f"      consumed:       {n_records_consumed} "
          f"({100*n_records_consumed/max(1,n_records_total):.1f}%)")
    print(f"      expired:        {n_records_expired} "
          f"({100*n_records_expired/max(1,n_records_total):.1f}%)")
    print(f"      active (end):   {n_records_active}")
    print(f"    pickups:          {len(cog._pickup_log)}")
    print(f"      uncontested:    {n_pickup_uncontested}")
    print(f"      contested:      {n_pickup_contested}")
    if all_bonuses:
        print(f"    ttl_bonus stats:")
        print(f"      max:  {max(all_bonuses)}")
        print(f"      mean: {np.mean(all_bonuses):.2f}")
        print(f"      n>0:  {sum(1 for b in all_bonuses if b > 0)}/{len(all_bonuses)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tau", type=int, default=V913_TAU_DEFAULT,
                        help="persistence threshold (age_r >= tau で label 候補)")
