...
以下を「追加のみ」で実装 (Layer C = CID self-read, additive-only):
  ...
  - Fetch Operations (pulse タイミングでの read_own_state 呼び出し、Lazy Registration)
  - A observer (per_subject CSV への v915_* 6 列追加、per_cid_self CSV 新規出力)
...
(B を read-only でのみ参照)。v915_cid_self_buffer.py は [B]、
v915_a_observer.py / v915_divergence_tracker.py は [A]。

出力ディレクトリ: diag_v915_{tag}/
...
USAGE:
  # smoke
  python v915_memory_readout.py --seed 0 --maturation-windows 5 \\
      --tracking-windows 2 --window-steps 100 --tag smoke

  # 本番 (24 seeds)
  seq 0 23 | parallel -j24 python v915_memory_readout.py \\
...
from v915_fetch_operations import ensure_cid_registered as v915_ensure_cid_registered
from v915_fetch_operations import pulse_fetch as v915_pulse_fetch
# v9.15: A-side observer (read-only view of CidSelfBuffer)
from v915_a_observer import (
...
from v915_divergence_tracker import (
...
def run(seed=42, maturation_windows=20, tracking_windows=10,
    ...
    # pulse タイミングで read_own_state を呼ぶ。
    ...
    for tw in range(tracking_windows):
        ...
        for step in range(window_steps):
            ...
            for cid_p in list(cog.all_hosted_cids()):
                ...
                # ──────────────────────────────────────────────
                # v9.15 Layer C: CID self-buffer Fetch (pulse タイミング)
                # state / engine / RNG を一切 mutate しない read-only 操作。
                # registry に未登録の cid は no-op (False 返し、続行)。
                # ──────────────────────────────────────────────
                v915_pulse_fetch(
                    v915_self_buffers, cid_p,
                    engine.state, engine.state.alive_l, t_cum)
    ...
    for cid, meta in subject_meta.items():
        ...
        subject_rows.append({
            ...
            **(lambda _bg=cog.v11_b_gen.get(cid),
            ...
            # ──────────────────────────────────────────────
            # v9.15: CID self-buffer summary (末尾 6 列、既存列は改変しない)
            # ──────────────────────────────────────────────
