#!/usr/bin/env python3
"""
ESDE v9.8a — Subject Reversal Only
====================================
GPT 監査メモ ESDE_v9_8_Design_Audit_Memo (Phase v9.8a) に基づく実装。

目的:
  認知層の主体を label_id (lid) から cognitive_id (cid) に反転させる。
  cid は単調増加 int で再利用しない。
  label cull 時に cog データを削除せず、ghost 状態として保持する。
  ghost は perception 更新を停止する (inert)。
  TTL 10 window 経過で reap (provisional retention limit).

含まないもの (v9.8a 範囲外):
  - 内省 (v9.8b 予定)
  - 再宿主化 (v9.8c 予定)
  - v9.7 の z-score torque modulation (反射型フィードバック、v9.7 で限界露呈)

親バージョン: primitive/v96/v96_baseline.py
変更箇所: CognitiveLayer → SubjectLayer (cid 主キー)、
         label birth/cull 時の hook、ghost 管理、出力カラム追加

USAGE:
  # Single seed
  python v98a_subject_reversal.py --seed 42 --tracking-windows 10

  # 48 seeds, short run (Run A)
  seq 0 47 | parallel -j24 python v98a_subject_reversal.py --seed {} --tracking-windows 10

  # 5 seeds, long run (Run B)
  seq 0 4 | parallel -j5 python v98a_subject_reversal.py --seed {} --tracking-windows 50 --tag long
"""
...
def run(seed=42, maturation_windows=20, tracking_windows=10,
    ...
    outdir = Path(f"diag_v98a_subject_{tag}")
    ...
    # ════════════════════════════════════════════════════════
    # SAVE Layer B (新): per_subject CSV (v9.8a 新規)
    # ════════════════════════════════════════════════════════
    subject_rows = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="ESDE v9.8a Subject Reversal (no feedback, no introspection)")
