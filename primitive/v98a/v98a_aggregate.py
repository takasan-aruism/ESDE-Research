#!/usr/bin/env python3
"""
ESDE v9.8a — Subject Reversal Results Aggregator
==================================================
Combines per-seed CSVs and JSONs into unified summaries.
Adds subject-layer metrics (cid count, hosted/ghost split, ghost duration).
...
If sibling baselines exist, also runs comparison:
  ...
  All three available           → 3-way summary at the end

USAGE (from primitive/v98a):
  python v98a_aggregate.py --tag short
  python v98a_aggregate.py --tag long

Layout assumed:
  primitive/v96/diag_v96_baseline_{tag}/   ← v96 baseline (optional)
  primitive/v97/diag_v97_feedback_{tag}/   ← v97 feedback (optional)
  primitive/v98a/diag_v98a_subject_{tag}/  ← v98a subject reversal (required)
  primitive/v98a/v98a_aggregate.py         ← THIS FILE
"""
...
def run(tag="short"):
    # v98a (required)
    v98a_base = Path(f"diag_v98a_subject_{tag}")
    ...
    # v9.8a-specific report
    _report_subject_layer(v98a_metrics, tag)
    ...
    if v97_base.exists():
        ...
        if v97_metrics:
            _compare_pair(v98a_metrics, v97_metrics, "v98a", "v97", tag)
    else:
    ...
    if v96_base.exists():
        ...
        if v96_metrics:
            _compare_pair(v98a_metrics, v96_metrics, "v98a", "v96", tag)
    else:
        print(f"\n  NOTE: v96 not found at {v96_base}")

    # 3-way summary if all available
    if v97_metrics and v96_metrics:
        print(f"\n{'='*70}")
        print(f"  3-WAY SUMMARY  v96 / v97 / v98a   ({tag})")
        print(f"{'='*70}\n")
        ...
        for name, m in [("v96", v96_metrics), ("v97", v97_metrics),
            ...
            print(f"    {name:>8}  {r if r is not None else 'n/a':>12}  "
                  f"{c if c is not None else 'n/a':>8}  "
                  f"{d if d is not None else 'n/a':>8}")
        ...
        for key, name in [("dist_social_mean", "social"),
            ...
            v96v = v96_metrics.get(key)
            v97v = v97_metrics.get(key)
            v98av = v98a_metrics.get(key)
            print(f"    {name:>12}: "
                  f"v96={v96v if v96v is not None else 'n/a':>8}  "
                  f"v97={v97v if v97v is not None else 'n/a':>8}  "
                  f"v98a={v98av if v98av is not None else 'n/a':>8}")
        ...
        print(f"\n{'='*70}")
        print(f"  END 3-WAY SUMMARY")
        print(f"{'='*70}\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="ESDE v9.8a Subject Reversal Aggregator (+ v96/v97 comparison)")
