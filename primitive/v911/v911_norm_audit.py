#!/usr/bin/env python3
"""
ESDE v9.11 Norm Audit — disposable measurement script
======================================================
v9.10 (v910_pulse_model.py) からコピーし、計測パッチのみ追加。
目的: v9.11 の V11_NORM_N を決定するため、pulse 発火時の
n_local = len(struct_set) の分布を実測する。

変更箇所 (すべて「追加のみ」、v9.10 既存ロジック / 出力は bit identical):
  - outdir を "diag_v911_norm_audit_{tag}" に変更 (v9.10 出力と衝突回避)
  - current_struct_size dict (cid -> int) を 1 step ごとに更新
  - pulse 発火時に n_local を v911_n_local_log に記録
  - n_local_audit_seed{seed}.csv を追加出力

削除・変更しているもの: なし (v9.10 の CSV は同じ内容で出力される)
この script は v9.11 本実装では使わない (Step 0 専用の使い捨て)。

USAGE:
  python v911_norm_audit.py --seed 0 --maturation-windows 20 \\
      --tracking-windows 5 --window-steps 500 --tag norm_audit
...
_V910_DIR = _REPO_ROOT / "primitive" / "v910"   # for virtual_layer_v9

for p in [str(_SCRIPT_DIR), str(_V82_DIR), str(_V43_DIR),
          str(_V41_DIR), str(_ENGINE_DIR), str(_V910_DIR)]:
...
def run(seed=42, maturation_windows=20, tracking_windows=10,
    ...
    outdir = Path(f"diag_v911_norm_audit_{tag}")
    ...
    # ── v9.11 norm audit: n_local measurement storage ──
    # Updated each step in the cognitive update loop; read at pulse firing.
    current_struct_size = {}   # cid -> int (len(struct_set) at latest step)
    v911_n_local_log = []      # list of dicts per pulse event
    ...
    for tw in range(tracking_windows):
        ...
        for step in range(window_steps):
            ...
            for lid in list(spatial_fields.keys()):
                ...
                # v9.11 norm audit: stash current struct_set size for pulse firing
                current_struct_size[cid] = len(struct_set)
            ...
            for cid_p in list(cog.all_hosted_cids()):
                ...
                ev = cog.on_pulse(cid_p, d_soc_p, d_sta_p, d_spr_p, d_fam_p,
                                  t_cum, w)
                # v9.11 norm audit: record n_local at every pulse firing
                # (regardless of cold_start, so we get the full distribution).
                # ev may be None for non-trigger pulses; pulse_n is still
                # incremented inside on_pulse. We read n_local from the
                # stash regardless.
                n_local_audit = current_struct_size.get(cid_p, 0)
                v911_n_local_log.append({
                    "seed": seed,
                    "cid": cid_p,
                    "t": t_cum,
                    "window": w,
                    "pulse_n": cog.v10_pulse_count.get(cid_p, 0),
                    "n_local": n_local_audit,
                    "ev_fired": 1 if ev is not None else 0,
                })
                if ev is not None:
                    row = {
                        "seed": seed,
    ...
    # ════════════════════════════════════════════════════════
    # SAVE (v9.11 norm audit): n_local measurement log
    # ════════════════════════════════════════════════════════
    csv_n_local = pulse_dir / f"n_local_audit_seed{seed}.csv"
    if v911_n_local_log:
        with open(csv_n_local, "w", newline="") as f:
            writer = csv.DictWriter(
                f, fieldnames=v911_n_local_log[0].keys())
            writer.writeheader()
            writer.writerows(v911_n_local_log)
        # Print distribution summary (informational)
        vals = np.array([r["n_local"] for r in v911_n_local_log])
        print(f"    --- v9.11 norm audit ---")
        print(f"    n_local samples: {len(vals)}")
        print(f"      min : {int(vals.min())}")
        print(f"      p10 : {float(np.percentile(vals, 10)):.2f}")
        print(f"      p50 : {float(np.percentile(vals, 50)):.2f}")
        print(f"      p90 : {float(np.percentile(vals, 90)):.2f}")
        print(f"      p95 : {float(np.percentile(vals, 95)):.2f}")
        print(f"      max : {int(vals.max())}")
        print(f"      mean: {float(vals.mean()):.2f}")
