...
00_index.md (このファイル)
  ↓ ★末尾の「用語対応表」を必ず最初に読む
07_concept_core.md      ← 哲学コア + Developmental 概念 (v10.2 対応)
  ↓
...
05c_primitive_summary_v913_addendum.md ← v9.13 追補
  ↓
06_developmental_summary.md ← ★ Developmental phase (v10.0/v10.1/v10.2、最新)
  ↓
08_esde_system_structure.md ← 現行システム構造 (v10.2 対応、4 層 + C 機構)
  ↓
09_audit_principles.md ← 監査原則 (v10.2 対応、二層 bit-identity、立場併存)
```

**急ぎなら**: 本ファイル末尾の **用語対応表** + `06_developmental_summary.md` + `08_esde_system_structure.md` + `07_concept_core.md` の Developmental 申し送り の組み合わせで現状作業 (v10.2 完了時点) に最低限着手できる。**ただし `04_cognition_summary.md` の「却下された方針」は時間を作って必ず読むこと**。
