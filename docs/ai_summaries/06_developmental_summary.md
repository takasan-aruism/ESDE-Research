...
| v10.3 | 双方向 E3 機構と Integration 登場条件の観察 | ✅ |
| v10.4 | 主題候補検討中 (Integration 独立化 / 嗜好 / focus 動的化 / 摂食順序) | 未着手 |
...
両者が hosted ∧ Q > 0 ∧ C ≥ 1 ∧ 同一 alive link 初回接触 onset。両者 C-1 を引く。

### 5.4 C 消費の意味づけ (Taka 整理 2026-04-29)

C 消費は「観察者が決めた記録ルール」であって、cid 内部選択の実装ではない。これにより観察者依存性を内包した系として ESDE が立ち上がる。

### 5.5 Integration 概念 (Taka 整理 2026-04-29)
...
### 5.6 第三項候補のリストアップ (10 カテゴリ)

Cat 1a-1c (cid 第三項)、Cat 2a (共有ノード)、Cat 3a-i/3b/3c (履歴)、Cat 4a-c (環境)。Cat 2b/2c は v10.4 以降、Cat 5 (cid 自己参照) は永久除外。
...
target 外も全体集計で監視 (bias 対策)。

### 5.8 本番 run 結果 (N=5000、24 seeds × tracking 50)

| 指標 | 値 |
|---|---:|
| 完走 | 24/24 |
| wall time mean | 2.97 h |
| 双方向 E3 fired | 6,824 件 |
| target 内 fired | 2,150 |
| target 外 fired | 4,674 |

**物理層 frozen の本番規模での確認**:
- labels 24/24 完全一致
- persistence 96/96 完全一致
...
C 消費による動学変化。**観察ルールが系の動学を変える** ことが定量的に確認。

### 5.10 第三項の構造的発見

- closed triad: 39 件 (1.4%)
- open triad: 2,788 件 (98.6%)
- 比率 116:1 で **open triad 主役性** を確認
...
### 6.9 ESDE の観察対象としての位置づけ (Taka 整理 2026-04-28)
...
### 8.1 v10.3 で確認された主たる観察対象

v10.3 本番 run で n_core=5 を含むペアが 50%+。主役同士の接触が be3 の主因。v10.4 でも n_core=4, 5 集団が中心、層化解析を継続。
...
post-ingestion weight bias、salience 駆動。
...
### 8.3 v10.3 で観察された軸の継承

v10.3 で確立した観察軸は v10.4 でも継承:
- 軸 1: 双方向 E3 の n_core 跨ぎパターン (v10.3 で「跨ぎが主」を確認)
- 軸 2: 主役 cid (n_consciousness ≥ 5) での観察
- 軸 3: 第三項候補のカテゴリ重なり数 (Integration 登場前兆)
- 軸 4: 持続性 (v10.3 で 0 件、v10.4 で Integration 機構により再評価)
- 軸 5: 系の動学 (C 蓄積、認知/意識バランス) の追加変化

### 8.4 マイルストーン: 研究の射程の拡張
...
v10.3 で Layer 5 (CID 共鳴) の入口が成立、長期射程に向けた具体的観察手段が立ち上がった。
...
### 8.6 v10.3 で確立された設計哲学の v10.4 への継承
...
Developmental Summary は、v9.x Primitive フェイズで言語化された意識の原資モデル (Q 消費 → 意識層 1 への転化) を v10.x で動作する機構として実装した記録の AI summaries 版であり、v10.0 のフェイズ宣言と 4 層アーキテクチャ確定、v10.1 の摂食機構導入と「物質的なもの」概念の発見、v10.2 の認知/意識確率バランス機構と n_core 別の戦略二極化観察、v10.3 の双方向 E3 機構と Integration 登場条件の観察を通じて、単一の物理規則と確率規則から複数の生存戦略が内発的に生成され物理層から意識層まで構造的に継承され、cid 個別の系から cid 集団の系 (Layer 5) への拡張入口が成立した記録を要約する。v10.3 で確認された「観察ルールが系の動学を変える」観察者依存性、open triad 主役性、持続性ゼロ (= 統計的痕跡としての三項共鳴) は v10.4 以降の Layer 5 本格実装の素材となる。

---

*以上、Developmental Summary。次の更新は v10.4 完了時。*
