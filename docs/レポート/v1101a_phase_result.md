...
v11.0.1.a 段階 1 は注意機構が ESDE スケールで実際に emit を出した最初の主題であり、Code A Step B-H 全 7 段階完了・物理層 frozen 完全保証 (v106/v107/v105 main 1,097 files 不変、bit-identity 3 層全 PASS)・新規 main run 不要 (既存出力流用) のうえで、核心観察として「意識優位 (conscious_dominant) のとき注意候補の波及 influence_candidate_count が認知優位の 1.54-1.78 倍、6 構造単位すべてで同方向・ESDE 解像度系で倍率最大」が出た、これは Taka フレーム「意識層 = 選択と集中」と整合的だが Phase Result の解釈としては「意識優位という状態と波及の広さが連動する」という観察事実の確認に留め「選択と集中が立証された」とは言わない、副次に (1) 因果候補 path が attention_via_salience 76.5% / familiarity 23.5% / temporal 0.01% の 3 path に集中し integration_alpha/beta が最強 path として全 24 seeds で 0 件 — 注意の「どこから来たか」を見ると Integration 経路が一度も立たない構造的事実 (留保 #L5、本主題で最も注視すべき発見)、(2) 意識優位時に因果候補の familiarity 経路が +6% (認知優位 19.1% → 意識優位 25.4%) — 箱 1「連想ゲーム = 直前の認知的固定を踏み台にした既知概念への連想」と方向が一致する構造的裏付け候補 (留保 #L6)、(3) predecessor 連鎖 (箱 1) が全 6 構造単位で成立 (埋まり率 86.6-100%) し「霧の中の意識だけ」状態を構造的に禁止する設計が動いた、(4) seed 0 は他 seed より控えめだが方向の反転はなく強化のみ (留保 #L3 集計単位による方向変動の継承、v1101 #33 と同型) が確認され、Phase Result の総括は「注意機構は ESDE スケールで成立し、意識優位と波及の連動・連想ゲームの方向的裏付け・Integration 経路の不可視という 3 つの観察を残した」であり、段階 2 (cid state ledger 再生・時間軸付き観察) を進めるかは留保 #L5 と #L6 が時間軸なしでは確かめきれない点を主たる判断材料として Taka が決める。

---

## 1. 段階 1 が示したこと — 構造の確認

### 1.1 注意機構が ESDE スケールで動いた
...
### 3.1 Integration 経路が因果候補として一度も立たない (留保 #L5)

本主題で最も注視すべき観察事実だと考える。

因果候補 (causality_candidate_path、注意候補が「どこから来たか」の最強経路) の分布は、attention_via_salience 76.5% / familiarity 23.5% / temporal_coactivation 0.01% / integration_alpha 0% / integration_beta 0%。Integration 経路 (α・β) が最強 path として全 24 seeds で一度も出現しない。

原因は構造的で bug ではない。v10.7 の relation_strength の出方 — cid レベルの mass-weighted event の数 (salience) が、構造的接続のみの Integration 経路に強さで常に勝つ。

なぜこれを注視するか。本主題は v1101 の「観察単位ごとに像が割れる」を起点にし、Integration (α/β) を 6 構造単位の 2 つとして並べた。qc_ratio も波及も Integration 単位で出ている。ところが注意の「どこから来たか」を見ると、Integration は因果として一度も見えない。Integration は注意の対象としては存在するのに、注意の由来としては不可視。

これは段階 1 の観察事実として記録すべきもので、判定ではない。だが Phase Result としては、これが何を意味しうるかの候補を挙げておく。(a) v10.7 の strength 定義が salience に偏っているための計測アーティファクトで、Integration 経路は実際には効いているのに強さ比較で負けているだけ。(b) ESDE の注意は実際に cid レベルの salience 連鎖で駆動されていて、Integration はその結果を集約する単位ではあっても注意の由来ではない。(a) と (b) は段階 1 では区別できない。区別するには relation_strength の定義を Integration 経路について見直すか、時間軸を入れて「Integration 経路が遅れて効く」possibility を見る必要がある。これは段階 2 または次主題の論点。
...
| integration paths 0 件出現の v10.5/v10.7 設計との関係 | §3.1。計測アーティファクト (a) か注意が実際に salience 駆動 (b) か段階 1 では区別不能。本主題で最も注視すべき観察事実として留保 #L5 |
| 留保 #L5/L6/L7 candidate の本主題位置づけ | §6。#L5 を主要留保に格上げ、#L6 を段階 2 動機に接続、#L7 は #L3 と統合 |
...
留保 #L5 の Integration 経路 0 件は、時間軸を入れれば「Integration 経路が遅れて効く」possibility が見える。段階 1 は run 全体の static path map なので、遅延型の効きは原理的に見えていない。
...
段階 2 の 1.5-2 日を投じる前に、段階 1 の観察を次主題の起点にする道もある。特に留保 #L5 (Integration 経路の不可視) は、段階 2 の時間軸付き観察より、v10.7 の relation_strength 定義そのものを Integration 経路について見直す別主題の方が筋がよい可能性がある。

Web Claude の見立て: 段階 2 に進むなら、その出口を「核心観察の選択と集中 / 拡散の切り分け」と「留保 #L6 連想ゲームの連鎖確証」に固定するのがよい。留保 #L5 (Integration 経路) は段階 2 でなく別主題の方が適切かもしれない。段階 2 を「とりあえず細かく見る」で始めると、設計書 §6 の出口固定原則 (絶対格言 #6) から外れる。進む / 進まないは Taka 判断だが、進む場合は出口を上記 2 点に絞ることを推奨する。
...
| **#L5** | **Integration 経路が因果候補として全 24 seeds で 0 件** | **§3.1。本主題で最も注視すべき観察事実に格上げ。段階 2 または別主題の論点** |
...
v11.0.1.a 段階 1 は、注意機構を ESDE スケールで実際に走らせた最初の主題であり、物理層 frozen を完全に保ったまま、注意の派生記録を 6 構造単位 × 3 変化指標で出した。核心観察として「意識優位と注意候補の波及の広さが連動する」が 6 構造単位すべてで頑健に出た — Taka フレーム「意識層 = 選択と集中」と方向が一致するが、選択と集中という機能の同定には至っていない。副次に 3 つの観察を残した — 注意の由来として Integration 経路が一度も立たないという構造的事実 (留保 #L5、本主題で最も注視すべき発見)、意識優位時に familiarity 経路が増えるという連想ゲームの方向的裏付け候補 (留保 #L6)、そして「霧の中の意識だけ」を禁止する箱 1 の predecessor 連鎖が実際に動いたこと。段階 1 は v1101 の「観察単位ごとに像が割れる」の続きとして、その像の差分 (変化) に注意を向ける形が描けることを示した。段階 2 に進むなら出口を「核心観察の選択と集中 / 拡散の切り分け」と「連想ゲームの連鎖確証」に固定すべきで、留保 #L5 は別主題の方が適切な可能性がある。主題評価と段階 2 の要否は Taka の判断領域。

---

## 8. 一文サマリ (再掲)

本書は v11.0.1.a「ESDE スケール注意機構」段階 1 の Phase Result であり、Code A Step H が judgement なしで記録した観察事実の解釈統合として、段階 1 が注意機構を ESDE スケールで物理層 frozen を保ったまま走らせた構造の確認・核心観察「意識優位と波及の広さの連動 (認知優位の 1.54-1.78 倍、6 構造単位同方向)」を「選択と集中の立証」でなく「連動の観察事実」として置き・副次観察 3 点 (留保 #L5 Integration 経路が因果候補として全 24 seeds 0 件 = 本主題で最も注視すべき構造的事実 / 留保 #L6 意識優位時 familiarity +6% = 連想ゲームの方向的裏付け候補 / predecessor 連鎖が箱 1 設計どおり全 6 構造単位で成立) を整理し・seed 0 は控えめだが方向の反転なし (留保 #L3) を確認し・Code A が Web Claude 領域として渡した 4 点に §2-3 で回答し・段階 2 (cid state ledger 再生・時間軸付き観察) を進めるなら出口を「核心観察の選択と集中/拡散切り分け」と「連想ゲーム連鎖確証」に固定すべき・留保 #L5 は別主題が適切な可能性ありと判断材料を出した、主題評価と段階 2 要否は Taka 判断領域。
