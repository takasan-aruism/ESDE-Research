# ESDE Cognition Report (Final)

*Phase: Cognition (v3.0–v4.9) → Circulation (v5.0–v5.1) → Recurrence (v6.0–v6.1) → World Induction (v7.0–v7.4)*
*Status: COMPLETE. v7.4 で大規模検証完了。次フェーズ Autonomy へ移行。*
...
| v7.3 | 2026-03-19 | Taka→Claude (Impl, Ryzen) | Metabolism: budget=1, zero-sum share allocation | **200 win: 1120 born→9 survived (0.8%); labels are phase-frequencies not spatial clusters; R=2.0 cycle spans 2 labels (inter-label bridge); birth law discovered (late+calm+5 nodes)** |
| v7.4 | 2026-03-21 | Taka→Claude (Impl, Ryzen) | Large-scale: N=10000, 100 steps, 12 seeds | **12 seed 全て同一均衡 (links 5701±100, sI 0.986±0.011); R+≈8 は N にスケールしない (N=5000 でも N=10000 でも同じ); vLb≈17 (N にサブリニア); 動的平衡は seed 非依存(普遍); engine_accel v3 (key/get_latent patch) 導入** |

---

*Cognition phase complete. v3.0–v7.4 を通じて確立されたこと: (1) 物理層は N=5000–10000 で動的平衡を自律維持する (stress_intensity ≈ 1.0, seed 非依存)。(2) 仮想層は budget=1 の代謝で label を圧縮淘汰する (1120→9, 97% 圧縮)。(3) label は空間クラスタではなく位相周波数グループ (θ-space の組織化)。(4) R>0 (triad) は label 間の境界に出現する。(5) R+ ≈ 8 は N にスケールしない — 構造の複雑さは N ではなくルールから生まれる。(6) 12 seed で全指標が一致 — 以上は普遍的性質。Cognition が残した問い: budget=1 の仮想エネルギーから label 間相互作用 (スケール転移) をどう生むか。label 単体ではありえない存在が label の集合から生まれる条件は何か。これが Autonomy phase の課題。*
