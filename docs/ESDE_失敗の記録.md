...
| **M** | **検出力/構造を測らず null を不在と読む** | 実装は算術的に忠実でも、計器の検出力（小標本 tail のノイズ床）や lens の構造（ICC）を測らずに「効果なし」を結論。「negative でも発見」の事前登録枠が null 無批判受容の免罪符になる。 | v1304c 固定 probe＝M=20 参照で「前提ずれ床内」と誤断→pooled 220 監査で反転（link 両 base p≤0.001）/ R_density は ICC=0.02 で組成が母集団を動かせず＝構造上結果の出ない lens 選定（memory `feedback_measurement_power_before_null`） |
...
### 2.6 フロンティア（v12 Atomset / v13 child-world / Attention Phase v1301+）
詳細は技術仕様書 §17。**注**: v1301 以降は 2026-07-04 に「Attention Phase」と遡及命名（史実は不変・§17 冒頭対応表）。
| 版 | 何を試したか | どう失敗 | 原因型 |
|---|---|---|---|
...
| v1304 Stage3 smoke | unpaired の子集団比較で親特異乖離を断定 | 過大な noise 床（unpaired・非対 seed）で inconclusive→撤回し paired+resample へ組み直し | **C/D（unpaired 過大 noise 床で断定しない・対で組む）** |
| v1304c 固定 probe | premise_drift を M=20 参照で primary に | tail 珍しさの推定ノイズが信号を埋め「前提ずれ床内」と誤断→pooled 220 監査で反転（link 両 base p≤0.001）。「negative も発見」枠が null 無批判受容の免罪符に | **M（検出力を測らず null を不在と読む）** |
| v1304c lens 選定 | R_density を自己確認外し lens に採用 | ICC=0.02 で組成が母集団を動かせず前提ずれ原理的ゼロ＝構造上結果の出ない実験 | **M/C（lens の ICC を事前確認）** |
| v1304 機構仮説（Web Claude） | 「動的珍しさは負フィードバックで s_avg 管に効く」物語 | 即検証で corr(注意, チャネル)=+0.115（逆向き）→仮説棄却。**物語で埋めず即反証した良い例**（反面の規律実践） | （反例＝J/L の解毒: 物語化せず即検証） |

> **v1304a は失敗でなく境界確定**: 「動的な注意は one-shot では子を親特異に作らない（NO・パイプは通る）」は次段（ループ v1304b）への正しい分岐であり、失敗カタログでなく §17.6 の観察事実。
...
| 「前提ずれ床内」の null | M=20 参照の tail 推定ノイズ | 参照母集団を pool で確保（220+）・母集団 mean/分布距離の低ノイズ計器・**null 前に検出力を測る** |
| 組成効果 lens の空振り | ICC≈0（cid が値を決めない）lens | lens の **ICC を事前確認**・ICC≈0 は構造的に結果が出ない |

> 共通解毒: **shuffle/置換対照**・**効果サイズ**・**n_core 層化**・**観測窓と指標が処置に感応するか**・**肯定結果ほど交絡を疑う**・**null 前に検出力と ICC を測る**。
...
## 5. 一文まとめ
ESDE 開発の失敗は、**(A) 神の手・(B) 物理層介入・(C) 観察方法が結果を作る・(D) 平均化の罠・(E) 層/主体の混同・(F) 異系対応**の 6 主因（+ G timescale/spatial mismatch・H 洗濯機・I 配線盲点・J 運用・K 直感の外れ・L 意味盛り）が全フェーズで反復したもので、Cognition v3-v7 の「物理層は床」「選択なき循環は洗濯機」が物理 frozen 規律の起源、Primitive の n_core/固定閾値アーティファクトと Unified の異系対応 4 連続失敗が「観察方法を疑う／同系内で測る」規律の起源、フロンティア v13 の run 長トートロジー・空エンジン交絡・#CW7 が「肯定結果ほど交絡を疑う」の最新例、**Attention Phase v1304c の検出力アーティファクト（M=20 参照で「前提ずれ床内」と誤断→pooled 監査で反転）と ICC≈0 lens の構造的空振りが「null を結論する前に検出力と構造 ICC を測る」（新型 M）の起源**であり、いずれも**失敗そのものより、それが生んだ規律（神の手回避・物理 frozen・shuffle 対照・効果サイズ・n_core 層化・上位資料参照・smoke で判定しない・null 前に検出力/ICC を測る）こそが ESDE の堅牢性の実体**である。
