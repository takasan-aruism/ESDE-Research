...
class ChemistryParams:
    ...
    E_yield_syn: float = 0.0    # energy bonus on Synthesis (A+B→CC)
    E_yield_auto: float = 0.0   # energy bonus on Autocatalysis (C+A→CC)
    ...
    def step(self, state: GenesisState) -> List[ReactionRecord]:
        ...
        for r in matched:
            ...
            if r.rule == 1:  # Synthesis: A+B → C+C
                state.Z[r.node_i] = 3
                state.Z[r.node_j] = 3
                # v1.6: Synthesis yield
                if self.params.E_yield_syn > 0:
                    state.E[r.node_i] = min(1.0, state.E[r.node_i] + self.params.E_yield_syn)
                    state.E[r.node_j] = min(1.0, state.E[r.node_j] + self.params.E_yield_syn)
                ...
                if state.Z[r.node_i] == 3 and state.Z[r.node_j] == 1:
                    ...
                    state.Z[r.node_i] = 3
                # v1.6: Autocatalysis yield
                if self.params.E_yield_auto > 0:
                    state.E[r.node_i] = min(1.0, state.E[r.node_i] + self.params.E_yield_auto)
                    state.E[r.node_j] = min(1.0, state.E[r.node_j] + self.params.E_yield_auto)
            used_nodes.add(r.node_i)
