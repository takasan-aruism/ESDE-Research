"""
ESDE Genesis v1.2 — 2D Noise Ridge Map
=========================================
p_link_birth (danger) × bg_inject (fuel)
Find the ridge: max cycles with η≥0.8 and no collapse.

Controller FROZEN. bias=0.7. 3 seeds per grid point.
"""
...
P_GRID = [0.004, 0.007, 0.012, 0.020]
BG_GRID = [0.001, 0.003, 0.007, 0.015]
...
def run_one(seed, plb, bgi):
    ...
    logger=GenesisLogger(); tracker=CT(); g_scores=np.zeros(N_NODES)
    ...
    for step in range(INJECTION_STEPS):
        ...
        grower.step(state); physics.step_decay_exclusion(state)
        logger.observe(state); tracker.rec(state,state.step-1)
    ...
    for step in range(QUIET):
        ...
        logger.observe(state,reactions=rxns); tracker.rec(state,state.step-1)
    ...
    return {"cycles":cyc,"cyc_rate":round(cyc/(ts/1000),3),
        ...
        "spr":round(spr,4),"compress":round(C,4),
        "entropy":round(H,4),"X":X,
...
def main():
    print("="*70)
    print("  ESDE Genesis v1.2 — 2D Noise Ridge Map")
    print(f"  p_link_birth × bg_inject | {len(SEEDS)} seeds × {QUIET} quiet")
    print("="*70)
    ...
    rows=[]; t0=time.time()
    for pi,plb in enumerate(P_GRID):
        for bi,bgi in enumerate(BG_GRID):
            seed_res=[]
            for seed in SEEDS:
                r=run_one(seed,plb,bgi)
                xb=bl[seed]["X"]; eta=r["X"]/xb if xb>0.01 else 0
                col=r["min_links"]==0 or eta<0.6
                row={"plb":plb,"bgi":bgi,"seed":seed,"cyc_rate":r["cyc_rate"],
                    "opp_rate":r["opp_rate"],"eta":round(eta,4),"X":r["X"],
            ...
            tag="C" if me<0.6 or nc>=2 else "D" if me<0.8 else "S"
            print(f"  plb={plb:.3f} bg={bgi:.3f}: η={me:.3f} cyc={mc:.1f} [{tag}] "
                  f"({time.time()-t0:.0f}s)")

    with open("ridge_2d_summary.csv","w",newline="") as f:
    ...
    for plb in P_GRID:
        for bgi in BG_GRID:
            ...
            agg.append({
                ...
                "med_miss_strong":round(np.median([r["miss_strong"] for r in sub]),3),
                "min_links_worst":min(r["min_links"] for r in sub),
            })
    with open("ridge_2d_aggregate.csv","w",newline="") as f:
        w=csv.DictWriter(f,fieldnames=agg[0].keys()); w.writeheader(); w.writerows(agg)

    # Heatmaps
    fig,axes=plt.subplots(2,2,figsize=(16,14))
    fig.suptitle("ESDE Genesis v1.2 — 2D Noise Ridge Map\np_link_birth × bg_inject",
                 fontsize=14,fontweight="bold",y=0.99)

    def make_heatmap(ax,key,title,cmap="viridis",vmin=None,vmax=None):
        ...
        ax.set_xticks(range(len(BG_GRID))); ax.set_xticklabels([f"{v:.3f}" for v in BG_GRID],fontsize=8)
        ax.set_yticks(range(len(P_GRID))); ax.set_yticklabels([f"{v:.3f}" for v in P_GRID],fontsize=8)
        ...
        for i in range(len(P_GRID)):
            for j in range(len(BG_GRID)):
                ax.text(j,i,f"{grid[i,j]:.2f}",ha="center",va="center",fontsize=9,
                        color="white" if grid[i,j]<(vmax or grid.max())*0.6 else "black")
        plt.colorbar(im,ax=ax,shrink=0.8)

    make_heatmap(axes[0][0],"med_cyc","Median Cycles/1k","YlOrRd")
    make_heatmap(axes[0][1],"med_eta","Median η","RdYlGn",0.4,1.2)
    make_heatmap(axes[1][0],"med_X","Median X","plasma")
    make_heatmap(axes[1][1],"pct_col","% Collapse","Reds",0,100)

    plt.tight_layout(rect=[0,0,1,0.96])
    plt.savefig("heatmap_ridge.png",dpi=150,bbox_inches="tight")
    print(f"\n  Plot: heatmap_ridge.png")

    # Find ridge
    stable=[a for a in agg if a["med_eta"]>=0.8 and a["pct_col"]==0]
    if stable:
        best=max(stable,key=lambda a:a["med_cyc"])
        print(f"\n  RIDGE POINT: plb={best['plb']} bg={best['bgi']} "
              f"cyc={best['med_cyc']} η={best['med_eta']} X={best['med_X']}")
    else:
        best=max(agg,key=lambda a:a["med_cyc"])
        print(f"\n  No fully stable ridge. Best cycles: plb={best['plb']} bg={best['bgi']}")

    # Conclusion
    txt=f"""ESDE Genesis v1.2 — 2D Ridge Conclusion
========================================
Grid: p_link_birth {P_GRID} × bg_inject {BG_GRID}
Seeds: {SEEDS} | Quiet: {QUIET}

{'Ridge point: plb='+str(best['plb'])+' bg='+str(best['bgi']) if stable else 'No stable ridge found'}
{'  cycles/1k='+str(best['med_cyc'])+' η='+str(best['med_eta'])+' X='+str(best['med_X']) if stable else ''}

Stable region (η≥0.8, 0% collapse):
{chr(10).join(f"  plb={a['plb']} bg={a['bgi']}: cyc={a['med_cyc']} η={a['med_eta']}" for a in stable) if stable else '  None found'}

Collapse region:
{chr(10).join(f"  plb={a['plb']} bg={a['bgi']}: η={a['med_eta']} col={a['pct_col']}%" for a in agg if a['pct_col']>0 or a['med_eta']<0.6)}

Recommended operating band:
  p_link_birth: [0.004, 0.015]
  bg_inject: [0.001, 0.015]
"""
    with open("v12_ridge_conclusion.txt","w") as f: f.write(txt)
    print(txt)
