...
class GenesisState:
    EXTINCTION = 0.001  # v0.6: Ghost Links — lowered from 0.01
    ...
    def __init__(self, n_nodes: int, c_max: float = 1.0, seed: int = 42):
        ...
        # v0.5: Chemical state  0=Dust, 1=A, 2=B, 3=C
        ...
        # v0.8: Latent link potential (sparse cache)
        self.L: Dict[Tuple[int, int], float] = {}
        self._latent_rng = np.random.RandomState(seed + 1000)

        # v1.7: Node fertility field (static topography)
        # F_i modulates latent refresh rate. Mean=1.0, variance controlled externally.
        self.F: np.ndarray = np.ones(n_nodes)  # flat by default
    ...
    # ---- v0.5: Chemistry helpers ----

    def state_counts(self) -> Dict[int, int]:
        """Count nodes in each chemical state (among alive nodes)."""
        counts = {0: 0, 1: 0, 2: 0, 3: 0}
        for i in self.alive_n:
            counts[self.Z[i]] = counts.get(self.Z[i], 0) + 1
        return counts

    def nodes_in_resonant_loops(self) -> Set[int]:
        """Return set of node IDs that participate in any resonant link."""
        nodes = set()
        for k in self.alive_l:
            if self.R.get(k, 0) > 0:
                nodes.add(k[0])
    ...
    # ---- v0.8: Latent Link Methods ----

    def get_latent(self, i: int, j: int) -> float:
        """Get latent potential for pair (i,j). Initialize lazily if unseen."""
        k = self.key(i, j)
        if k not in self.L:
            # Deterministic init based on pair identity
            self.L[k] = self._latent_rng.random()
        return self.L[k]

    def set_latent(self, i: int, j: int, val: float):
        k = self.key(i, j)
        self.L[k] = max(0.0, min(1.0, val))

    def latent_mass(self) -> float:
        """Sum of all stored L values / 2 (symmetric)."""
        return sum(self.L.values()) / 2.0
