...
==========================================
Phase : Ecology (first experiment)
...
observer regions, and measure inter-region active-link structure over time.
...
def aggregate():
    ...
    for res in results:
        rk = " ".join(f"{res['regions'][f'r{r}']['dominant_k']:>3}"
                       for r in range(N_REGIONS))
        # Mean inter-region edges
        ie = res.get("inter_region", {})
        total_ie = sum(v["mean_count"] for v in ie.values())
        print(f"  {res['seed']:>5} | k={res['global_dominant_k']} "
              f"{res['global_switches_per_100']:>5.1f} | {rk} | {total_ie:.0f}")
