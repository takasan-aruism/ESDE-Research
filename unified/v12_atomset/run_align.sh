...
for s in $SEEDS; do python3 unified/v12_atomset/m5_substrate_atom.py A "$s" "$TRACK" align 1 "$ROOT" > "/tmp/m5al_A_s${s}.log" 2>&1 & done
for c in B D E; do for s in $SEEDS; do
  python3 unified/v12_atomset/m5_substrate_atom.py "$c" "$s" "$TRACK" align 1 "$ROOT" > "/tmp/m5al_${c}_s${s}.log" 2>&1 & done; done
