...
for s in $SEEDS; do
  python3 unified/v12_atomset/m5_substrate_atom.py A "$s" "$TRACK" torque > "/tmp/m5ac_A_torque_s${s}.log" 2>&1 &
done
# C, F: 各チャネル
for ch in $CHANS; do
  for c in C F; do
    for s in $SEEDS; do
      python3 unified/v12_atomset/m5_substrate_atom.py "$c" "$s" "$TRACK" "$ch" \
