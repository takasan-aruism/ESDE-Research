#!/usr/bin/env python3
"""v1105a Step G — bit-identity 3 層検証

層 A: v1105a Step C-F 再実行で hash 一致 (8 ファイル)
層 B: v10.5/6/7 + v112 + v1101a/1102/1103/1104/1104a/1105 全 frozen (1,494)
層 C: v1105a scripts の書込みパスが unified/v1105a/ 配下のみ
"""
...
V1105A_DIR = REPO_ROOT / "unified/v1105a"
V1105A_MAIN = V1105A_DIR / "outputs/main"

LAYER_A_FILES = [
    "trial_step2_associations.parquet",
    "trial_step3_distributions.parquet",
    "trial_step4_labels.parquet",
    "trial_step4_distributions.parquet",
    "trial_summary_metrics.parquet",
    "trial_layer_jaccard.parquet",
    "trial_source_layer_overlap.parquet",
]
LAYER_A_RERUN = [
    ("v1105a_step_c_trial_step1_2.py", []),
    ("v1105a_step_d_trial_step3.py", []),
    ("v1105a_step_e_trial_step4.py", []),
    ("v1105a_step_f_aggregate.py", []),
]
V1105A_SCRIPTS = [
    "v1105a_step_b_env_check.py",
    "v1105a_step_c_trial_step1_2.py",
    "v1105a_step_d_trial_step3.py",
    "v1105a_step_e_trial_step4.py",
    "v1105a_step_f_aggregate.py",
...
ALLOWED_V1105A = ("V1105A", "OUT_MAIN", "OUT_DIR", "out_path", "out", "out_sum",
                   "out_jacc", "out_olap", "out_labels", "out_dist")
...
def layer_a():
    ...
    pre = {f: sha(V1105A_MAIN / f) for f in LAYER_A_FILES if (V1105A_MAIN / f).exists()}
    for script, args in LAYER_A_RERUN:
        t = time.time()
        subprocess.run([sys.executable, str(V1105A_DIR / script)] + args,
                       capture_output=True, check=True, cwd=str(REPO_ROOT))
        r["rerun_sec"][script] = round(time.time() - t, 2)
    post = {f: sha(V1105A_MAIN / f) for f in LAYER_A_FILES}
...
def _under(expr):
    return any(c in expr for c in ALLOWED_V1105A)


def layer_c():
    f = []
    for s in V1105A_SCRIPTS:
        text = (V1105A_DIR / s).read_text()
...
def main():
    ...
    report = {"version": "v11.0.5a (v1105a) Step G",
              "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
              "layer_a": la, "layer_b": lb, "layer_c": lc,
              "all_layers_pass": all_pass}
    out = V1105A_DIR / "v1105a_step_g_bit_identity_report.json"
