#!/usr/bin/env python3
"""v1106 Step G — bit-identity 3 層検証

層 A: v1106 Step C-F 再実行で hash 一致 (8 ファイル)
層 B: v10.5/6/7 + v112 + v1101a-v1105a 全 frozen + Synapse データ frozen (1,520)
層 C: v1106 scripts の書込みパスが unified/v1106/ 配下のみ
"""
...
V1106_DIR = REPO_ROOT / "unified/v1106"
V1106_MAIN = V1106_DIR / "outputs/main"

LAYER_A_FILES = [
    "observation_1_synset_distributions.parquet",
    "observation_1_labels.parquet",
    "observation_1_excluded_fnd_spaceless.json",
    "observation_2_synapse_alignment.parquet",
...
LAYER_A_RERUN = [
    ("v1106_step_c_observation_1.py", []),
    ("v1106_step_d_observation_2.py", []),
    ("v1106_step_e_observation_3.py", []),
    ("v1106_step_f_observation_4.py", []),
]
V1106_SCRIPTS = [
...
ALLOWED_V1106 = ("V1106", "OUT_MAIN", "OUT_DIR", "out_path", "out", "out1", "out2", "out3")
...
def layer_a():
    ...
    pre = {f: sha(V1106_MAIN / f) for f in LAYER_A_FILES if (V1106_MAIN / f).exists()}
    for script, args in LAYER_A_RERUN:
        t = time.time()
        subprocess.run([sys.executable, str(V1106_DIR / script)] + args,
                       capture_output=True, check=True, cwd=str(REPO_ROOT))
        r["rerun_sec"][script] = round(time.time() - t, 2)
    post = {f: sha(V1106_MAIN / f) for f in LAYER_A_FILES}
...
def _under(expr):
    return any(c in expr for c in ALLOWED_V1106)


def layer_c():
    f = []
    for s in V1106_SCRIPTS:
        text = (V1106_DIR / s).read_text()
...
def main():
    ...
    report = {"version": "v11.0.6 (v1106) Step G",
              "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
              "layer_a": la, "layer_b": lb, "layer_c": lc,
              "all_layers_pass": all_pass}
    out = V1106_DIR / "v1106_step_g_bit_identity_report.json"
