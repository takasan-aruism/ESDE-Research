...
## 観察対象注釈ブロック
同系内（child=親 seed0 CID の縮小版、異系対応でない）。読=frozen（per_subject_seed0・v19g_canon・仕様書）。書=`unified/v1301/` のみ。child engine は in-memory、親 physics/inject/ledger/state 非書込。**本 run は seed=3 smoke（承認後 seed 拡張）**。
