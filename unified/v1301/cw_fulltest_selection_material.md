...
→ 素の記述: **「10 個の独立な CID 値」ではない**。相関した CID 値を別々の物理 param に当てると **実質同じ軸を複数回入れる＝交絡**。ただし次元数は **pooled と stratum 内で大きく違う**（下記★訂正）:

### ★ 自己懐疑チェックで判明した訂正（pooled の罠・M_c4 共線・stratum 依存）
1. **pooled の低次元は n_core 階層の産物**: 上の formed 85 の **PC1(46%) は n_core とほぼ同一**（corr(PC1, n_core)=**0.91**）。「規模10束」も中身は n_core 軸（B_gen≒n_core）。**pooled で低次元に見えるのは n_core を混ぜたから**で、純 CID 値の次元ではない。
2. **n_core 固定 stratum の真の次元はもっと高い**: n_core=2（n=54, n_core 固定で清浄）で PC1=**35%**（pooled 46% より低い＝支配軸が弱い）、5軸で 64%、**Kaiser 14 軸**、90% に 16 軸。＝純 CID 値の独立軸は **~14 程度（pooled の見かけより高い）**。n5(17) は n=17・p=65 で **PCA 次元推定不能**（母数不足）。
3. **M_c4 値は独立でなく、共線ペアが stratum で変わる**（前回「3値が独立軸」は**誤り**）:
   - n_core=2: **B_gen↔S_avg=0.82**（共線）、r_core・phase_sig 独立。
   - n_core=5: **B_gen↔r_core=−0.94**（共線・Spearman−0.93・drop-one[−0.96,−0.92]＝頑健）、S_avg↔B_gen=0.48、phase_sig 独立。
   - → **phase_sig だけは両 stratum で一貫して独立**。残り3値は stratum ごとに違う共線構造。**既存4-knobテストも n5 で B_gen→N と r_core→K_sync に −0.94 相関入力を使っていた**（N がほぼ定数 CV0.025 ゆえ効果が muted だっただけ＝隠れた交絡）。
   - → **「独立代表を1度選べば全 n_core で使える」は成り立たない**。選定は n_core ごとにやり直す必要。
...
→ **B_gen は n_core にほぼ連動**（n_core 内で横並び、跨ぐと段階的）。**★訂正: 「5000ノード」は親(v918 main run)の N=5000 のことで、child の目標値ではない**。child の N は設計上 **B_gen×10 ≈ 110〜354**（小規模）で、5000 を目指す設計は元々ない（前回「5000未達」は親と child を混同した誤フレーミング）。B_gen は桁で振れない（cross-n_core で max/min 3.2 倍）＝n_core 跨ぎで N は約3倍動くが桁ではない。**n_core=3 は 3 CID しかなく相関/検定に不足**。
...
| 案 | 内容 | 長所 | 短所 |
|---|---|---|---|
| **案A 独立軸代表** | §1 相関クラスタごとに代表1値を選び、§2 独立な物理軸（~6-7）に1対1で当てる | **交絡最小**・効果が状態変数ごとに分離可・物理軸数(~6-7)と整合・解釈明快 | 「全検」でない（束内の特定値を捨てる＝代表選択の恣意）・束内で個別に効く値を見逃す恐れ・**★共線構造が n_core で変わるため代表選択を n_core ごとにやり直す必要**（n2:B_gen-S_avg / n5:B_gen-r_core） |
...
## 一文サマリ
全検 選定合理性の判断材料（Code A、2026-06-21、調査のみ・実行ゼロ・選定確定なし。自己懐疑チェックで3点訂正済）── **§1 CID 値は「10 独立値」ではない**が、次元数は **pooled と stratum で違う**: pooled formed85 の低次元(PC1=46%)は **n_core 階層の産物**(corr(PC1,n_core)=0.91)で純 CID 構造でない。**n_core 固定 stratum(n2,n=54)の真の独立軸は ~14**(PC1=35%・Kaiser14)。**M_c4値は独立でなく共線ペアが stratum で変わる**(n2:B_gen-S_avg0.82 / n5:B_gen-r_core−0.94、phase_sig のみ一貫独立、既存4-knobも隠れ交絡)。**§2 物理 param も独立でなく状態変数(L/θ/S/E/R/Z)で束ねられ独立軸 ~6〜7**(S 過剰決定、beta=R↔S 結合、Flow=θ→E 結合)。**§3 配線可能 ~25 knob だが knob 数はコストを増やさない**(driver=CID×対照×seed×step、合議前提「204×knob」は誤り)、n_core 跨ぎ母集団 85(2:54/3:3/4:11/5:17)、**「5000ノード」は親 v918 の N で child 目標でない**(child N=B_gen×10≈110-354)、seed12・全跨ぎ・2ratio で **~12h で回る**。**§4 基準案 A(独立軸代表・ただし n_core ごとに選定し直す要)/B(構造同型＋pairing 吸収)/C(純全検・冗長交絡)＋ハイブリッド**。核心: 独立軸 ~6-7 が実体ゆえ 25 knob は冗長、real≒shuffle 回避に pairing 検定が前提、コストは knob でなく CID×seed、M_c4 すら共線で phase_sig 以外は stratum 依存。選定確定・実行はせず 3AI 合議＋Taka へ。
