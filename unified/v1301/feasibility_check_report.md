...
- param 導出＝親 CID 形態の**構造特徴を read-only で読み param に写すのみ**（実現値コピーでない＝死線回避、前ターン §3 と整合）。書込は child engine（in-memory）と新規 `unified/v1301/` のみ。親物理 非書込。
...
- 一方向: 読＝frozen（per_subject_seed0 / source_events_seed0 / atom_profiles_cache / build_pulse_cid_vector）。書＝`unified/v1301/` のみ。親 physics/inject/state/ledger 非書込。
