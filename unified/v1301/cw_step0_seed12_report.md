...
## 観察対象注釈ブロック
同系内（child=親 seed0 CID の縮小版）。読=frozen。書=`unified/v1301/` のみ。child in-memory、親非書込。*実行*: 2026-06-21、`cw_step0.py` CW_SEEDS=12、**1002 child（real 984 + canon 18）、run_len=35000、Pool24、5977s（1.66h）**。出力 = `cw_step0_signatures.parquet`(上書き) / `cw_step0_summary.json`。
...
- 一方向: 読=frozen。書=`unified/v1301/` のみ。child in-memory、親非書込。
