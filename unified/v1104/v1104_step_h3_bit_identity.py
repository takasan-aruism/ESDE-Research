#!/usr/bin/env python3
"""v1104 Step H-3 bit-identity 再検証

Step G を拡張し、再調査 4 件出力 (5 parquet) も決定論的であることを確認。
層 A: Step B-E + Step H-3 再実行 → parquet hash 一致
...
def main():
    ...
    print("[A] re-run Step B-E + Step H-3, hash match")
    la = layer_a()
    ...
    report = {"version": "v11.0.4 (v1104) Step H-3",
              "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
              "layer_a": la, "layer_b": lb, "layer_c": lc,
              "all_layers_pass": all_pass}
    out = V1104_DIR / "v1104_step_h3_bit_identity_report.json"
