#!/usr/bin/env python3
"""v1104 Step G — bit-identity 3 層検証

層 A: Step B-E 再実行 (smoke level、seed 0 のみ) → parquet hash 一致
層 B: v10.5 / v10.6 / v10.7 / v1101a / v1102 / v1103 main outputs 全 frozen
層 C: 段階 2 scripts 書込みパス unified/v1104/ 配下のみ
"""
...
# Step E (観察 4) は短時間で全 24 seeds 再実行可能
# Step B (観察 1) は 19s、Step C は 55s、Step D は 7s — full re-run
LAYER_A_RERUN = [
...
def main():
    ...
    report = {"version": "v11.0.4 (v1104) Step G",
              "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
              "layer_a": la, "layer_b": lb, "layer_c": lc,
              "all_layers_pass": all_pass}
    out = V1104_DIR / "v1104_step_g_bit_identity_report.json"
