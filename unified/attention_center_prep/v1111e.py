#!/usr/bin/env python3
"""v1111e 第一段 — 共通を統計で固める (24 atom seed で §2.1 inversion 再現確認)

Web Claude v1111e 設計準拠:
- 配管は v1111d のまま (出口 phase 分布 + 一致率)
- 24 atom seed に拡張
- 主役 §2.1 atom 横断一貫性 inversion 一本
- §2.2/§2.3 は記録だが判定不使用 (二段手順、土台なしで局所読まない)
- 192 tasks Pool(24) で 8 Wave 並列、推定 1.5-2 時間
"""
import os
...
# 24 seeds (v1111e 第一段、memory 24 seeds 1 バッチ規律)
ATOM_SEEDS = list(range(1000, 1024))    # 1000-1023
CENTER_SEEDS = list(range(2000, 2024))  # 2000-2023 (Atom と 1:1 対応)
...
def main():
    print('=== v1111e 第一段 — 24 atom seed で §2.1 inversion 再現確認 ===\n')
    ...
    print(f'  並列: Pool(24)、推定 {len(tasks) // 24} Wave × ~13 分 = ~{len(tasks)*13//24} 分\n')
    ...
    # === §2.1 atom 横断一貫性 (主役、24 atom 全ペア cos = 276 pairs) ===
    print('\n=== §2.1 atom 横断一貫性 (主役、24 atom × 全ペア = 276 pairs) ===')
    consistency_records = {}
    ...
    if n_inversion == 3:
        print(f'  → 強再現性 (3/3): §2.1 共通土台確定候補 → 第二段へ')
    elif n_inversion == 2:
        print(f'  → 中再現性 (2/3): v1111d 3 atom と同等')
    elif n_inversion == 1:
        print(f'  → 弱再現性 (1/3): Other 単独')
    else:
        print(f'  → 不再現 (0/3): 共通でも届かず')
    ...
    print(f'  9 サブグループ (3 group × 3 Other) 中の inversion 数: {n_inv_sub}/9')

    # cos 弱さ記録 (Web Claude §3)
    print('\n=== cos 絶対値 (弱さ、判定不使用、記録のみ) ===')
    ...
    print(f'  gap (shuffled - injected): {overall_shuf_cos - overall_inj_cos:+.4f}')
    print(f'  (両者 cos ~1 なら方向 seed 共通でも強さは弱い、第二段以降論題)')

    # v1111d との比較
    print('\n=== v1111d (3 atom) との比較 ===')
    print(f'  v1111d: injected 0.965 / shuffled 1.019 / gap +0.054 / 3 atom')
    print(f'  v1111e: injected {overall_inj_cos:.4f} / shuffled {overall_shuf_cos:.4f} '
          f'/ gap {overall_shuf_cos - overall_inj_cos:+.4f} / 24 atom')

    # §2.2 / §2.3 は記録だが判定不使用 (Web Claude §4 二段手順)
    # parquet に保存だけ
    print('\n=== §2.2 / §2.3 は記録のみ (二段手順、判定不使用) ===')
    # §2.2 (簡易記録)
    between_records = []
    for cond in ['injected_other', 'shuffled_other']:
        dp_per_so_by_atom = {}
        for sa in ATOM_SEEDS:
            dp_per_so_by_atom[sa] = {so: np.array(next(d['dp'] for d in dp_records
                                                          if d['atom_seed']==sa
                                                          and d['condition']==cond
                                                          and d['other_seed']==so))
                                       for so in OTHER_SEEDS}
        # 各 atom 内で Other 間 cos
        for sa, dp_per_so in dp_per_so_by_atom.items():
    ...
    between_df = pd.DataFrame(between_records)
    between_df.to_parquet(OUT_DIR / 'between_recorded_only.parquet', index=False)
    ...
    sf_df = pd.DataFrame(self_floor_records)
    sf_df.to_parquet(OUT_DIR / 'self_floor_recorded_only.parquet', index=False)
    print(f'  §2.2, §2.3 を parquet 保存 (判定不使用、第二段で参照)')
    ...
    print(f'\n=== v1111e 第一段 完了 total {time.time()-t_main:.1f}s ===')
