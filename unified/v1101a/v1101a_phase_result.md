...
| **#L5** | **Integration 経路が因果候補として全 24 seeds で 0 件** | **§3.1。Code A 追加調査で原因判明 — relation_strength の binary (integration 1.0 固定) と連続値 (salience/familiarity) を Step E が sum argmax で不当比較。仮説 A (観測器の問題) / B (階層の役割分担) 両方真。z-score 正規化で integration が 29 source_cid で出現・dominance 逆転を確認。Taka 判断 (iii) Step E に sum argmax + z-score argmax を両方併記。新バージョンを切らず v1101a 内の Step E 修正課題として対応 (原因が観測器の集約ロジックと判明、主題級でない)。段階 2 とは独立** |
