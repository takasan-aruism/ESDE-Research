...
for s in $SEEDS; do python3 unified/v1201/m5_substrate_atom.py A "$s" "$TRACK" core 1 "$ROOT" > "/tmp/m5co_A_s${s}.log" 2>&1 & done
for c in C F D E; do for s in $SEEDS; do
  python3 unified/v1201/m5_substrate_atom.py "$c" "$s" "$TRACK" core 1 "$ROOT" > "/tmp/m5co_${c}_s${s}.log" 2>&1 & done; done
