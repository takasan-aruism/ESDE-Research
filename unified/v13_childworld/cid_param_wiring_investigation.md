...
## 観察対象注釈ブロック
各 child-world は親 CID ごとの独立子系。観察＝各 child の同系内動学＋集計。param 導出＝親 CID 形態の構造特徴を read-only で読むのみ（実現値コピーでない＝死線回避）。書込は child engine（in-memory）と `unified/v13_childworld/` のみ。親 physics/inject/ledger/state 非書込。
...
- 一方向: 読＝frozen（per_subject_seed0 / 現行 engine 構成 / v19g_canon）。書＝`unified/v13_childworld/` のみ。親 physics/inject/ledger/state 非書込。
