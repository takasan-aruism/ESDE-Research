...
## 観察対象注釈ブロック
読＝frozen（`primitive/v918/diag_v918_main/subjects/per_subject_seed0.csv` の n_core=5 17 CID／`childworld_signatures_lifespan.parquet`／`cw_run_lifespan.py` の写像式）。書＝本文書 + `unified/v13_childworld/` のみ。再 run なし（既存データ + 写像式の検証のみ）。親 physics/inject/ledger/state 非書込。
...
- 一方向: 読＝frozen（per_subject_seed0・既存 parquet・写像式）。書＝本文書 + `unified/v13_childworld/` のみ。親 physics/inject/ledger/state 非書込。
