...
def run_one(seed, rate, quiet_steps=QUIET_STEPS):
    ...
    island_births=0; island_deaths=0; turnover_vals=[]
    boundary_events=0; swap_total=0
    island_lifetimes=defaultdict(int); dead_lifetimes=[]
    runaway=False
    ...
    for step in range(quiet_steps):
        ...
        if (step+1)%WINDOW==0:
            ...
            cur_islands=find_islands_sets(state)
            island_counts.append(len(cur_islands))

            cur_set=set(cur_islands); prev_set=set(prev_islands)
            matched_cur=set(); matched_prev=set(); pairs=[]
            for ci in cur_set:
                ...
                if best_j and best_ov>=2:
                    pairs.append((ci,best_j)); matched_cur.add(ci); matched_prev.add(best_j)
            island_births+=len(cur_set-matched_cur)
            island_deaths+=len(prev_set-matched_prev)
            for ci,pj in pairs:
            ...
            for ci in cur_set:
                ...
                if ci in matched_cur:
                    for c2,p2 in pairs:
                        if c2==ci: new_active[iid]=island_lifetimes.get(id(p2),0)+1; break
                else: new_active[iid]=1
            for pid in prev_set-matched_prev:
                dead_lifetimes.append(island_lifetimes.get(id(pid),1))
            island_lifetimes=new_active

            all_isl_nodes=set()
            for isl in cur_islands: all_isl_nodes|=isl
    ...
    rigidity=float(med_life/(1+mean_turn)) if (1+mean_turn)>0 else 0
    ...
    return {
        ...
        "boundary_crossing_1k":round(boundary_events/(quiet_steps/1000),2),
...
def aggregate():
    ...
    for rate in RATE_GRID:
        ...
        agg.append({
            ...
            "med_bnd_cross":round(np.median([r["boundary_crossing_1k"] for r in sub]),2),
            ...
            "med_swaps":round(np.median([r["swap_events"] for r in sub]),0),
        })
    ...
    print(f"{'='*70}")
    print(f"\n  {'rate':>7} {'cyc':>6} {'typ':>4} {'H':>6} {'top1':>5} {'isl':>5} "
          f"{'coex':>6} {'turn':>6} {'rig':>5} {'bnd':>6} {'X':>6} {'swaps':>6}")
    for a in agg:
        print(f"  {a['rate']:>7.4f} {a['med_cyc']:>6.1f} {a['med_types']:>4.1f} "
              f"{a['med_ent']:>6.3f} {a['med_top1']:>5.3f} {a['med_island']:>5.2f} "
              f"{a['med_coex']:>6.3f} {a['med_turnover']:>6.3f} {a['med_rigidity']:>5.2f} "
              f"{a['med_bnd_cross']:>6.1f} {a['med_X']:>6.4f} {a['med_swaps']:>6.0f}")
    ...
    pl(axes[1][0],"med_turnover","Island Turnover","#3498db")
    ...
    txt=f"""ESDE Genesis v1.8 — Boundary Intrusion Conclusion
=================================================
...
Best diversity: rate={best_div['rate']}
  ...
  turnover={best_div['med_turnover']} rigidity={best_div['med_rigidity']}

Global cycle types observed: {len(all_types)}
{chr(10).join(f'  {t}: {c}' for t,c in all_types.most_common(10))}

Rigidity vs Diversity:
{chr(10).join(f'  rate={a["rate"]}: rig={a["med_rigidity"]:.2f} types={a["med_types"]} turn={a["med_turnover"]:.3f}' for a in agg)}
