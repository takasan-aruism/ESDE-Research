...
# v10.4: ledger / observation_target は v104 版を使う (be3_fired_callback 追加)
from v104_spend_audit_ledger import SpendAuditLedger
from v104_observation_target import ObservationTargetTracker
# v10.4 Integration: independent subject that aggregates Q/C from member cids
from v104_integration import IntegrationManager
...
def run(seed=42, maturation_windows=20, tracking_windows=10,
    ...
    for sub in ["aggregates", "labels", "subjects",
                ...
                "integration"]: # v10.4: integration logs
    ...
    # v10.2: balance_rng を渡して確率決定機構を有効化
    # v10.3: observation tracker + bidirectional E3 機構を有効化
    v103_target_tracker = ObservationTargetTracker()
    # v10.4: Integration Manager を作成し、ledger に be3_fired callback として接続。
    # Integration は be3 fired pair を起点に Trigger A/B/C/D を判定する。
    # 観察対象 tracker にも参照を渡し、Integration 構成 cid を target に追加 (§11)。
    v104_integration_mgr = IntegrationManager(
    ...
    v914_ledger = SpendAuditLedger(
        ...
        be3_disable=False,
        be3_fired_callback=v104_integration_mgr.on_be3_fired)
    ...
    for tw in range(tracking_windows):
        ...
        window_rows.append({
            ...
            # v10.4 Integration: window 末で update する (一旦 0 で予約)
            "n_integrations_active": 0,
            "n_integrations_recorded": 0,
            "n_integrations_born": 0,
            "n_integrations_state_transitioned": 0,
            "total_q_inherited": 0,
            "total_c_inherited": 0,
            "total_q_distributed": 0,
            "total_c_distributed": 0,
            "max_integration_size": 0,
            "mean_integration_size": 0.0,
            "trigger_type_dist": "{}",
        })
    ...
    for cid, meta in subject_meta.items():
        ...
        subject_rows.append({
            ...
            **build_v918_subject_columns(v915_self_buffers, cid),
            # ──────────────────────────────────────────────
            # v10.4 Integration: per cid 集計 (§10.4)
            # ──────────────────────────────────────────────
            "n_integrations_joined":
                int(v104_integration_mgr.cid_n_integrations_joined.get(cid, 0)),
            "n_integrations_currently":
                int(len(v104_integration_mgr.cid_to_integrations.get(cid, ()))),
            "q_received_from_integrations":
                int(v104_integration_mgr.cid_q_received_from_integrations.get(cid, 0)),
            "c_received_from_integrations":
                int(v104_integration_mgr.cid_c_received_from_integrations.get(cid, 0)),
            "q_inherited_to_integration":
                int(v104_integration_mgr.cid_q_inherited_to_integration.get(cid, 0)),
            "c_inherited_to_integration":
                int(v104_integration_mgr.cid_c_inherited_to_integration.get(cid, 0)),
        })
    ...
    # ════════════════════════════════════════════════════════
    # v10.4 SAVE: Integration logs (integration/)
    # ════════════════════════════════════════════════════════
    integration_dir = outdir / "integration"
    integration_dir.mkdir(parents=True, exist_ok=True)

    # Logger 4: integration_lifecycle_log
    il_path = integration_dir / f"integration_lifecycle_log_seed{seed}.csv"
    il_fields = [
        "seed", "step", "integration_id", "event_type", "trigger_type",
        "member_cids", "q_total", "c_total",
        "q_inherited_delta", "c_inherited_delta",
    ]
    with open(il_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=il_fields)
        writer.writeheader()
        for r in v104_integration_mgr.lifecycle_log:
            writer.writerow({k: r.get(k, "") for k in il_fields})
    print(f"  v10.4 Integration: lifecycle_log: "
          f"{len(v104_integration_mgr.lifecycle_log)} rows -> {il_path}")

    # Logger 5: integration_distribution_log
    id_path = integration_dir / f"integration_distribution_log_seed{seed}.csv"
    id_fields = [
        "seed", "step", "integration_id", "target_cid",
    ...
    with open(id_path, "w", newline="") as f:
        ...
        writer.writeheader()
        for r in v104_integration_mgr.distribution_log:
            writer.writerow({k: r.get(k, "") for k in id_fields})
    print(f"  v10.4 Integration: distribution_log: "
          f"{len(v104_integration_mgr.distribution_log)} rows -> {id_path}")

    # Logger 6: integration_membership_log (run 末スナップショット)
    im_path = integration_dir / f"integration_membership_log_seed{seed}.csv"
    im_fields = ["seed", "step", "cid_id",
                 "integration_ids", "binding_strengths"]
    _membership_rows = v104_integration_mgr.get_membership_snapshot_rows(
        cumulative_step)
    with open(im_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=im_fields)
        writer.writeheader()
        for r in _membership_rows:
            writer.writerow({k: r.get(k, "") for k in im_fields})
    print(f"  v10.4 Integration: membership_log: "
          f"{len(_membership_rows)} rows -> {im_path}")

    # Integration run-level summary
    is_path = integration_dir / f"integration_summary_seed{seed}.csv"
    n_active_total = sum(
        1 for i in v104_integration_mgr.integrations.values()
        if i.state == "active")
    n_recorded_total = sum(
        1 for i in v104_integration_mgr.integrations.values()
        if i.state == "recorded")
    n_total_integrations = len(v104_integration_mgr.integrations)
    trigger_dist = Counter()
    for i in v104_integration_mgr.integrations.values():
        trigger_dist[i.trigger_type] += 1
    sizes_active = [
        len(i.member_cids) for i in v104_integration_mgr.integrations.values()
        if i.state == "active"]
    sizes_history = [
        len(i.member_history)
        for i in v104_integration_mgr.integrations.values()]
    is_fields = [
        "seed", "n_total_integrations",
        "n_active", "n_recorded",
        "n_birth_be3", "n_birth_open_triad",
        "n_birth_closed_triad", "n_birth_third_overlap",
        "max_active_size", "mean_active_size",
        "max_history_size", "mean_history_size",
        "total_q_inherited_run", "total_c_inherited_run",
        "total_q_distributed_run", "total_c_distributed_run",
    ]
    total_q_inh = sum(
        v104_integration_mgr.cid_q_inherited_to_integration.values())
    total_c_inh = sum(
        v104_integration_mgr.cid_c_inherited_to_integration.values())
    total_q_dist = sum(
        v104_integration_mgr.cid_q_received_from_integrations.values())
    total_c_dist = sum(
        v104_integration_mgr.cid_c_received_from_integrations.values())
    with open(is_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=is_fields)
        writer.writeheader()
        writer.writerow({
            "seed": seed,
            "n_total_integrations": n_total_integrations,
            "n_active": n_active_total,
            "n_recorded": n_recorded_total,
            "n_birth_be3": trigger_dist.get("be3", 0),
            "n_birth_open_triad": trigger_dist.get("open_triad", 0),
            "n_birth_closed_triad": trigger_dist.get("closed_triad", 0),
            "n_birth_third_overlap": trigger_dist.get("third_overlap", 0),
            "max_active_size": max(sizes_active) if sizes_active else 0,
            "mean_active_size":
                round(sum(sizes_active) / len(sizes_active), 4)
                if sizes_active else 0.0,
            "max_history_size": max(sizes_history) if sizes_history else 0,
            "mean_history_size":
                round(sum(sizes_history) / len(sizes_history), 4)
                if sizes_history else 0.0,
            "total_q_inherited_run": int(total_q_inh),
            "total_c_inherited_run": int(total_c_inh),
            "total_q_distributed_run": int(total_q_dist),
            "total_c_distributed_run": int(total_c_dist),
        })
    print(f"  v10.4 Integration summary: total={n_total_integrations} "
          f"(active={n_active_total}, recorded={n_recorded_total}) "
          f"trigger_dist={dict(trigger_dist)}")
