...
v10.4 main run (24 seeds × 50 windows × N=5000) の Layer B audit 全 5,224 cid から得られたデータ。

**全 cid の n_core 分布** (5,224 cids):

| n_core | 件数 | 比率 |
|---|---:|---:|
| **2** | **3,968** | **76.0%** (= 大多数の極小 cid) |
| 3 | 288 | 5.5% |
| 4 | 327 | 6.3% |
| 5 | 638 | 12.2% |
| 6, 7, 8 | 各 1 | <0.1% (極稀) |

**Integration 構成 cid の n_core 分布** (18,934 member slots = 13,550 件 × mean size):

| n_core | slot 件数 | 比率 | 全 cid 比率 | bias |
|---|---:|---:|---:|---:|
| 2 | 8,609 | 24.3% | 76.0% | **×0.32** (過少代表) |
| 3 | 2,320 | 6.6% | 5.5% | ×1.19 |
| 4 | 6,364 | 18.0% | 6.3% | **×2.87** (過大代表) |
| **5** | **17,993** | **50.8%** | **12.2%** | **×4.16** (極端な過大代表) |
| 6 | 47 | 0.1% | <0.1% | ×6.94 |
| 7 | 57 | 0.2% | <0.1% | ×8.41 |

**be3 (size 2) の組合せ Top** (7,085 件):

| 組合せ | 件数 | 比率 |
|---|---:|---:|
| **(2, 5)** | **1,861** | **26.3%** (= 小+大の非対称が最頻) |
| **(5, 5)** | **1,770** | **25.0%** |
| (4, 5) | 1,279 | 18.0% |
| (2, 4) | 623 | 8.8% |
| (3, 5) | 521 | 7.4% |
...
- 神の手なし (誕生条件は be3 fired のみ、n_core フィルタなし) で:
  - **n_core=5 cid は人口の 12% しかいないが Integration 構成員の 51% を占める** (×4.16 のハブ的活躍)
  - **n_core=2 cid は人口の 76% を占めるが Integration 構成員には 24% のみ** (×0.32、過少代表)
- (2, 5) と (5, 5) が並んで最頻 — 「**ハブ大 cid 同士の繋がり**」と「**ハブ大 cid が小 cid を巻き込む**」が並存
- Q/C 動学から自然にこの偏りが出る (実験者が事前にバイアスを設けていない)

→ 「**多ノード CID 同士が自然に繋がる**」+「**周辺の小 cid を巻き込む**」が α/β 議論の出発点。

### 0.4 size 3 (open_triad) 上位 5 パターンの詳細統計

5 パターンで合計 3,520 件 (全 size-3 の 67.6%)。明確に異なる性格を持つ:

#### ライフサイクル

| パターン | 件数 | 誕生 step (median) | recorded 率 | recorded まで (median) | 性格 |
|---|---:|---:|---:|---:|---|
| **(5, 5, 5)** | 675 | 8,413 | **1.8%** | **14,596** | **強い核** (最も長寿命) |
| (4, 5, 5) | 790 | 8,662 | 2.7% | 12,064 | 準安定核 |
| (2, 5, 5) | 920 | 8,577 | 5.2% | 10,539 | 捕獲型 |
| (2, 4, 5) | 601 | 8,408 | 7.3% | 8,221 | 橋渡し型 |
| **(2, 2, 5)** | 534 | 9,801 | **17.4%** | **7,775** | **散る周辺** (最も短命) |

→ **n_core が大きいほど Integration が長寿命**。(5,5,5) は 1.8% しか recorded にならず、(2,2,5) は 17.4% が recorded 化。

#### Q/C 継承と凍結率

| パターン | Q 継承計 | Q 凍結率 | C 継承計 | C 凍結率 |
|---|---:|---:|---:|---:|
| (2, 5, 5) | 106 | 34% | **465** | 98.1% |
| (4, 5, 5) | 50 | 72% | 180 | 97.2% |
| (5, 5, 5) | 84 | 81% | 393 | 95.2% |
| (2, 4, 5) | 103 | 62% | 355 | 94.9% |
| **(2, 2, 5)** | 10 | 20% | 10 | **100%** |

→ **C 凍結率は全パターンで 95-100%** = 継承された C はほぼ recorded に閉じ込められる
→ **(2,5,5) は C 継承量が最大 (465)** = 小 cid が ghost 化して C を吐き出す → 大 cid 集団が受け取る構図

#### 構成員の認知活動量 (Q_spent / spend_rate)

| パターン | mean Q0 | mean Q_spent / cid | spend_rate |
|---|---:|---:|---:|
| (5, 5, 5) | 33.3 | 31.55 | **94.6%** |
| (4, 5, 5) | 30.8 | 28.83 | 93.7% |
| (2, 5, 5) | 26.1 | 23.76 | 91.2% |
| (2, 4, 5) | 23.5 | 21.29 | 90.5% |
| **(2, 2, 5)** | 18.8 | 16.07 | **85.3%** |

→ **(5,5,5) は Q を 94.6% 使い切る** = 認知活動が活発、**(2,2,5) は 85% 使用で早く死ぬ** (15% の Q が未使用のまま消滅)

#### 構成員の重複所属 (1 cid が他 Integration にも所属する度合)

| パターン | uniq cids | 1 cid あたり所属数 mean | max |
|---|---:|---:|---:|
| **(5, 5, 5)** | 445 | **4.55** | **22** |
| (4, 5, 5) | 597 | 3.97 | 17 |
| (2, 5, 5) | 972 | 2.84 | 17 |
...
→ **(5,5,5) の構成員は同パターン内で平均 4.55 個に重複所属、最大 22 個** = ハブ大 cid が多数の (5,5,5) に同時参加
→ **(2,2,5) の小 cid は重複所属がほぼ無い (1.70)** = 1 回ポッキリの参加

#### 5 パターンの性格まとめ

| パターン | キーワード | 主機能 |
|---|---|---|
| (5, 5, 5) | **強い核** | 安定核、ハブ集中、長寿命、高 Q 使用 |
| (4, 5, 5) | 準安定核 | 中ハブ + 大ハブ、安定気味 |
| (2, 5, 5) | 捕獲型 | 大ペア + 小衛星、中継型 C 継承 |
| (2, 4, 5) | 橋渡し型 | 全段階混合、中間寿命 |
| (2, 2, 5) | 散る周辺 | 短命、後発、低活動 |
...
## 8. 上位 5 パターンの観察から見える α/β 比較の含意

### α 型での観察 (現状)

5 パターンが性格を保ったまま並存:
- (5,5,5) と (4,5,5) は安定核として長期 active を維持
- (2,5,5) は中継型として大量の C を継承
- (2,2,5) は短命周辺として高 recorded 率
- 「強い核」「捕獲型」「散る周辺」が異なる役割を**並列に演じる**

### β 型での予想 (B2 merge 規則採用時)

共有 member を持つパターン同士 (例: 同じ大 cid X が (5,5,5), (4,5,5), (2,5,5) 全てに登場) を merge すると:
- **「強い核 (5,5,5) を持続的中核に、(4,5,5) が補強、(2,5,5) (2,2,5) が周辺・流入として時系列的に取り込まれる」1 つの巨大階層型 Integration**
- 核 (大 cid 群) は recorded まで時間を要する → 巨大 Integration の連続性を保つ
- 周辺 (小 cid) は短命でメンバー入替えが頻発 → 巨大 Integration 内部での「出入り」が常時起こる
- 「核と縁辺の階層構造」が 1 つの単位の中で展開

### 共通する観察事実

α/β どちらでも:
- **C 凍結率 95-100%** = 「死者の意識資源」が記録として永続蓄積される現象
- recorded Integration は「忘れられない記憶集積体」として機能
- これは α 型で多数の小集積体に分散、β 型で少数の大集積体に集約、として異なる現れ方をする

---

## 9. 結論

v10.4 で実装した α 型は、**多数の小集団の並存** モデルとして機能している。観察された **5,224 cid の n_core=2 が 76% という大きな短命層** と、**Integration 構成では n_core=5 が ×4.16 で過大代表**という偏りは、α/β どちらの研究にとっても示唆深い:

- α 型: 5 パターンが多様な性格を演じる「**社会的多層**」モデル
- β 型: 核-周辺階層が 1 単位内で展開する「**個体的階層**」モデル

β 型 (統合成長モデル) は v10.5 以降の射程として実装可能。**merge 規則の選定 (推奨: B2)** と **trigger_type 起源情報の扱い** が主要な設計議論点。

α/β は **どちらが正しいか** ではなく、**何を観察したいか** によって選ぶべきモデル。両者を **同 seeds で並走比較** する設計が最も情報量が多い。

---

*以上、α/β 設計案 (詳細データ反映版)。3 AI + Web Claude + Taka 議論の素材として使う。*
