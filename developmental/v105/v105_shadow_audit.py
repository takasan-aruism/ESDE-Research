#!/usr/bin/env python3
"""ESDE v10.5 shadow audit wrapper.

実装指示書 §F.1 + §D.1: shadow audit (必須)
  N=5000、tracking 50、24 seeds
  shadow audit モードでの規律 (D1):
    - α/β-Integration: log のみ記録、実 Q/C 操作なし
    - Salience: OFF (適用しない、観察分布が歪むため)
    - Leakage: log のみ記録、実 C 転記なし
  C 消費・Q/C 継承・再分配・Leakage は記録のみ、実消費しない (be3_shadow_audit=True)

USAGE:
  seq 0 23 | parallel -j8 python v105_shadow_audit.py --seed {} --tag shadow
"""
import argparse
import sys
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="v10.5 shadow audit run")
