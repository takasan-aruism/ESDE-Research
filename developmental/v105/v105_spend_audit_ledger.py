...
class SpendAuditLedger:
    ...
    def __init__(self, delta_fn=None, disable_e3: bool = False,
                 ...
                 be3_fired_callback=None,
                 # v10.5 Salience / Leakage 拡張
                 ingestion_target_selector=None,
                 ingestion_post_callback=None) -> None:
        ...
        # v10.5 Salience: ingestion 候補から target を選ぶ callback。
        # signature: selector(observer_cid, candidates_sorted, fallback_rng,
        #                     global_step) -> chosen_idx (int)
        # None なら uniform random (= v10.4 互換)
        self._ingestion_target_selector = ingestion_target_selector
        # v10.5 Leakage: ingestion 成功後に呼ぶ callback
        # signature: callback(observer_cid, ghost_cid, global_step) -> None
        self._ingestion_post_callback = ingestion_post_callback
    ...
    def _run_ingestion_phase(self, candidates_per_observer: dict, *,
        ...
        for observer_cid in sorted(candidates_per_observer.keys()):
            ...
            if len(cands_sorted) == 1:
                chosen_idx = 0
            elif self._ingestion_target_selector is not None:
                # v10.5: Salience 適用 (selector が log と選定を行う)
                chosen_idx = int(self._ingestion_target_selector(
                    observer_cid=observer_cid,
                    candidates_sorted=cands_sorted,
                    fallback_rng=self._ingestion_rng,
                    global_step=global_step,
                ))
            else:
