"""
v9.17 段階 4 Layer C — CID Self-Buffer [B]
==========================================

CID が自分自身のメンバーノード/リンクを読み取るための専用メモリ領域。
研究者観察 (A) から分離された CID 主体 (B) 側モジュール。

段階 4 との差分 (vs 段階 3) — 指示書 §0.3:
  - 下層 (他者読み): E3_contact 発火時に相手の M_c 情報を部分取得する
    新規メソッド read_other_on_e3_contact を追加
  - other_records リスト + 統計カウンタ 3 本 (total_other_contacts /
    total_features_fetched / total_features_missing)
  - 他者読み専用 RNG (_build_local_rng_other_read) を分離
  - 相手の M_c features は _extract_M_c_features で抽出 (不変値のみ)
...
規約 (A/B 分離):
  - 本ファイルは [B]。他の [B] ファイル (v917_fetch_operations / v917_cid_view) のみ import 可
  - [A] ファイル (v917_a_observer, v917_divergence_tracker, v917_interaction_log)
    を import してはいけない
  - A 側への露出は `_a_observer_` 接頭辞メソッドの read-only API のみ

段階 4 禁止 (指示書 §1.1-1.2):
  - 研究者向け統計量 (mean / std / percentile) を持たない
  - 自分の member_nodes 以外 (自己読み) を超えて動的状態を読まない
  - 補完しない (欠損は missing のまま)
  - 他 cid の動的状態 (theta_current / Q_remaining 等) を保存しない
    (M_c の不変値のみ fetched_M_c に格納)
  - 他 cid の自己読み結果 (divergence 等) を取得しない (γ 禁止)
  - engine.rng を touch しない
  - InteractionLog を参照・書き込みしない (責務分離)
"""
...
# 段階 4 新規: M_c features 抽出ヘルパ (指示書 §4.5)
# 他者読みで相手の "不変値" のみをサンプリング対象とする。
# theta_birth 配列そのものは含めず、統計量 3 つのみ (Taka Q5 回答 2026-04-22)。
def _extract_M_c_features(cid_view) -> list:
    """
    CidView から M_c 相当の不変 features を (name, value) タプルの list で返す。

    固定で 10 項目 (順序も固定):
      B_Gen, Q0, n_core, S_avg_birth, r_core_birth, phase_sig_birth,
      theta_birth_mean, theta_birth_std, theta_birth_range, birth_step

    重要 (指示書 §1.2 #19-20, §4.5):
      - 不変値 (生誕時決定) のみ含める
      - 動的値 (Q_remaining, theta_current, age_factor 等) を含めない
      - theta_birth 配列そのものは含めず、統計量 3 つに集約 (Q5 回答)

    Parameters
    ----------
    cid_view : CidView (v917_cid_view.CidView)
        読む相手の read-only snapshot。
    """
    theta_birth = np.asarray(cid_view.theta_birth, dtype=float)
    if theta_birth.size > 0:
        theta_mean = float(np.mean(theta_birth))
        theta_std = float(np.std(theta_birth))
        theta_range = float(np.max(theta_birth) - np.min(theta_birth))
    else:
        theta_mean = 0.0
        theta_std = 0.0
        theta_range = 0.0
    ...
    return [
        ("B_Gen", float(cid_view.B_Gen)),
        ("Q0", int(cid_view.Q0)),
        ("n_core", int(cid_view.n_core)),
        ("S_avg_birth", float(cid_view.S_avg_birth)),
        ("r_core_birth", float(cid_view.r_core_birth)),
        ("phase_sig_birth", float(cid_view.phase_sig_birth)),
        ("theta_birth_mean", theta_mean),
        ("theta_birth_std", theta_std),
        ("theta_birth_range", theta_range),
        ("birth_step", int(cid_view.birth_step)),
    ]
...
class CidSelfBuffer:
    ...
    def __init__(
        ...
        # 段階 4 新規 (下層: 他者読み) — 指示書 §4.1
        # E3_contact 発火時に相手の M_c 情報を部分取得したログを append する。
        # 一度も接触がなければ [] のまま。records の内容は
        # read_other_on_e3_contact (Step 3) で確定する。
        self.other_records: list = []

        # 段階 4 統計カウンタ (A 向け集計用、指示書 §4.1)
        self.total_other_contacts: int = 0
        self.total_features_fetched: int = 0
        self.total_features_missing: int = 0
    ...
    def read_other_on_e3_contact(
        self,
        other_cid_view,
        current_step: int,
        event_id: int,
        seed: int,
        visible_ratio_override: float = None,
    ) -> None:
        """
        段階 4 下層 (指示書 §4.3): E3_contact 発火時に相手の M_c 情報を
        部分取得する。サンプリング量は相手の age_factor に比例 (候補 q)。

        Parameters
        ----------
        other_cid_view : CidView (v917_cid_view.CidView)
            相手 cid の read-only snapshot。Q_remaining はこの snapshot 時点の値。
        current_step : int — v913_global_step
        event_id : int — seed 内 0-based event index (main loop から渡される)
        seed : int — run の seed (独自 RNG 構築のみ)

        重要 (指示書 §1.2 禁止):
          - 相手の動的状態 (theta_current / Q_remaining 等) は保存しない
            (#20-21)。Q_remaining は visible_ratio 計算のみに使い、以降の
            CidSelfBuffer に残さない
          - 相手の自己読み結果 (divergence 等) は取得しない (#21, γ 禁止)
          - Fetch コスト 0 (Q 消費なし、#32)
          - engine.rng 非 touch (hash ベース独自 RNG)
          - InteractionLog を参照しない (責務分離)
        """
        # 1. 相手の age_factor (visible_ratio) を計算
        #    Q0=0 の相手は visible_ratio=0.0 (CidView.age_factor が clamp 済)
        # v10.5 Salience: visible_ratio_override が与えられたら mass-based
        # ブースト後の値を採用する (None なら v10.4 互換の age_factor)
        if visible_ratio_override is not None:
            visible_ratio = float(visible_ratio_override)
        else:
            visible_ratio = float(other_cid_view.age_factor)

        # 2. 相手の M_c features を列挙 (不変値 10 項目、固定順)
        other_M_c_features = _extract_M_c_features(other_cid_view)
        n_features = len(other_M_c_features)

        # 3. n_visible = round(n_features * visible_ratio) を [0, n_features] に clamp
        n_visible = int(round(n_features * visible_ratio))
        n_visible = max(0, min(n_features, n_visible))

        # 4. hash ベース独自 RNG (engine.rng 非 touch)
        local_rng = self._build_local_rng_other_read(
            seed=seed,
            self_cid_id=self.cid_id,
            other_cid_id=int(other_cid_view.cid_id),
        ...
        elif n_visible >= n_features:
            sampled_indices = list(range(n_features))
        else:
            sampled_indices = sorted(
                local_rng.sample(range(n_features), n_visible)
            )

        # 6. 取得 features と missing feature 名を分離
        sampled_set = set(sampled_indices)
        fetched_M_c = {
            other_M_c_features[i][0]: other_M_c_features[i][1]
            for i in sampled_indices
        }
        missing_feature_names = [
            other_M_c_features[i][0]
            for i in range(n_features) if i not in sampled_set
        ]

        # 7. 段階 4 統計カウンタを更新
        self.total_other_contacts += 1
        self.total_features_fetched += len(sampled_indices)
        self.total_features_missing += len(missing_feature_names)

        # 8. other_records に append (A 向け観測用、B 内では判定に使わない)
        self.other_records.append({
            "step": int(current_step),
            "other_cid_id": int(other_cid_view.cid_id),
            "event_id": int(event_id),
            "visible_ratio": visible_ratio,
            "sampled_feature_indices": list(sampled_indices),
            "fetched_M_c": fetched_M_c,
    ...
    def _build_local_rng(
        ...
        """
        engine.rng を touch しない独自 RNG (自己読み用)。
    ...
    def _build_local_rng_other_read(
        self,
        seed: int,
        self_cid_id: int,
        other_cid_id: int,
        ...
        """
        段階 4 新規: 他者読み用の hash ベース独自 RNG (engine.rng 非 touch)。
        指示書 §4.4 準拠。PYTHONHASHSEED 非依存 (明示式)。

        自己読み用 _build_local_rng と別メソッドにする理由:
          - シード構成要素が違う (event_type_full の代わりに other_cid_id + event_id)
          - 混同防止 (同じ step, seed でも自己読みと他者読みで別 RNG)
        """
        rng_seed_raw = (
            (int(seed) * 100003)
            ^ (int(self_cid_id) * 10007)
            ^ (int(other_cid_id) * 10009)      # 段階 4: 相手の cid_id を混ぜる
            ^ (int(current_step) * 131)
            ^ (int(event_id) * 31)
        )
        rng_seed = abs(rng_seed_raw) % (2**31)
