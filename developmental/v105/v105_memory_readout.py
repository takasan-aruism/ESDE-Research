...
# v10.5: ledger / observation_target は v105 版 (Salience / Leakage hook 追加)
from v105_spend_audit_ledger import SpendAuditLedger
from v104_observation_target import ObservationTargetTracker
# v10.5 Integration: α (観察軸) + β (会計単位) の階層分離
from v105_integration import IntegrationManagerV105 as IntegrationManager
# v10.5 Salience-driven Focus
from v105_salience import (
    SalienceTracker, compute_mass, adjust_visible_ratio,
    select_ingestion_target, log_be3_fired_event,
)
# v10.5 Historical Resource Leakage (recorded β からの漏れ)
from v105_historical_leakage import (
    LeakageTracker, trigger_leakage_be3, trigger_leakage_ingestion,
)
...
from v102_fetch_operations import ensure_cid_registered as v915_ensure_cid_registered
...
# v10.2 RNG seed magic for cognitive-conscious balance probability draw
# E3 onset で認知/意識の振り分けに使う。capture/ingestion とは独立 stream。
BALANCE_RNG_SEED_MAGIC = 0xBA1A2C     # seed ^ this for balance RNG

# v10.5 RNG seed magics (Salience / Leakage 用、既存 stream とは独立)
SALIENCE_RNG_SEED_MAGIC = 0x5A1E5CE   # mass-weighted observation
LEAKAGE_RNG_SEED_MAGIC = 0x1EAC0FF    # historical resource leakage (確率引きを使う場合)
...
class SubjectLayer:
    ...
    def __init__(self):
        ...
        # ──────────────────────────────────────────────
        # C[cid]: 意識層資源 (conscious_layer)。認知活動 (E3 認知) で +1、
        # 意識活動 (E3 意識発火) で -1。初期値 0、上限なし (Taka 確定 2026-04-26)。
        # birth で 0 で初期化、reap で pop。Layer A には書き込まない。
        self.C: dict = {}
...
def run(seed=42, maturation_windows=20, tracking_windows=10,
    ...
    for sub in ["aggregates", "labels", "subjects",
                ...
                "ingestion",    # v10.1: ingestion logs
                "audit",        # v9.14 Layer B audit
                "balance",      # v10.2: probabilistic balance logs
                "bidirectional", # v10.3: bidirectional E3 logs
                "integration", # v10.4: integration logs (α + β)
                "salience",    # v10.5: salience event log
                "leakage"]:    # v10.5: historical resource leakage log
    ...
    # v10.2 Probabilistic Balance: 確率決定用 RNG (engine/capture/ingestion と分離)
    # E3 onset で認知/意識のどちらが立つかの確率引きにのみ使用。
    balance_rng = np.random.default_rng(seed ^ BALANCE_RNG_SEED_MAGIC)

    # v10.5: Salience / Leakage 用 RNG (engine.rng 等とは独立 stream)
    salience_rng = np.random.default_rng(seed ^ SALIENCE_RNG_SEED_MAGIC)
    leakage_rng = np.random.default_rng(seed ^ LEAKAGE_RNG_SEED_MAGIC)  # 将来用

    # v10.5: Salience は shadow_audit 中は OFF (回答 D1)
    v105_salience_enabled = (not be3_shadow_audit)
    v105_leakage_enabled = True   # shadow_audit 中も log は取る (実 C 転記は内部で抑止)

    # v9.14 Step 1-3: Layer B ledger (audit-only, Layer A を一切触らない)
    # v10.5: ledger / observation_target / Integration / Salience / Leakage を統合
    v103_target_tracker = ObservationTargetTracker()
    # v10.5: α + β 統合 manager (cid 1 → β 1 規律、Q/C 継承は β に集約)
    v105_integration_mgr = IntegrationManager(
        seed=seed, target_tracker=v103_target_tracker)
    # v104 互換のエイリアス (以下の既存コードを最小変更で活かすため)
    v104_integration_mgr = v105_integration_mgr

    # v10.5: Salience tracker (mass-weighted observation の log + RNG)
    v105_salience_tracker = SalienceTracker(
        seed=seed, salience_rng=salience_rng,
        enabled=v105_salience_enabled)
    # v10.5: Leakage tracker (recorded β からの漏れ log)
    v105_leakage_tracker = LeakageTracker(seed=seed)

    # ingestion target selector (Salience 適用、ledger に渡す)
    def _ingestion_target_selector_fn(*, observer_cid, candidates_sorted,
                                       fallback_rng, global_step):
        return select_ingestion_target(
            observer_cid=observer_cid,
            candidates=candidates_sorted,
            cog=cog, ledger=v914_ledger,
            beta_manager=v105_integration_mgr.beta,
            salience_tracker=v105_salience_tracker,
            fallback_rng=fallback_rng,
            global_step=global_step,
            enabled=v105_salience_enabled,
        )

    # ingestion 後 callback (Leakage 発火)
    def _ingestion_post_callback_fn(*, observer_cid, ghost_cid, global_step):
        trigger_leakage_ingestion(
            observer_cid=observer_cid,
            ghost_cid=ghost_cid,
            beta_manager=v105_integration_mgr.beta,
            cog=cog,
            leakage_tracker=v105_leakage_tracker,
            global_step=global_step,
            shadow_audit=be3_shadow_audit,
            enabled=v105_leakage_enabled,
        )

    # be3 fired callback (Integration + Salience log + Leakage)
    def _be3_fired_chain_callback(*, cid_a, cid_b, window, step,
                                   global_step, cog, ledger):
        # v10.5: 順序は Integration → Salience log → Leakage
        # (Integration は β を更新、Salience は β 状態の mass を読む、
        #  Leakage は recorded β から漏れる)
        v105_integration_mgr.on_be3_fired(
    ...
    v914_ledger = SpendAuditLedger(
        ...
        be3_target_tracker=v103_target_tracker,
        be3_shadow_audit=be3_shadow_audit,
        be3_disable=False,
        be3_fired_callback=_be3_fired_chain_callback,
        ingestion_target_selector=_ingestion_target_selector_fn,
        ingestion_post_callback=_ingestion_post_callback_fn,
    )
    ...
    # ── Tracking Phase ──
    # v10.2: c_trajectory (per cid × per window) accumulator
    v102_c_trajectory_rows: list = []
    ...
    for tw in range(tracking_windows):
        ...
        # v10.2: window 開始時の balance_decisions index
        _balance_count_at_window_start = len(v914_ledger.balance_decisions)
        # v10.4: Integration window-scoped state (be3 edge graph) をリセット
        v104_integration_mgr.reset_window_state()
        ...
        for step in range(window_steps):
            ...
            for _idx_offset, v915_ev in enumerate(
                ...
                if _contacted_view is not None:
                    # v10.5 Salience: visible_ratio を mass-based に boost
                    _v105_visible_ratio = adjust_visible_ratio(
                        base_visible_ratio=float(_contacted_view.age_factor),
                        other_cid=_contacted_cid,
                        cog=cog, ledger=v914_ledger,
                        beta_manager=v105_integration_mgr.beta,
                        enabled=v105_salience_enabled,
                    )
                    # log Salience event (read_other)
                    if v105_salience_tracker is not None and v105_salience_enabled:
                        _v105_other_mass = compute_mass(
                            cid=_contacted_cid, cog=cog, ledger=v914_ledger,
                            beta_manager=v105_integration_mgr.beta,
                        )
                        v105_salience_tracker.log_event(
                            global_step=cumulative_step,
                            observer_cid=int(_observer_cid),
                            candidate_cids=[int(_contacted_cid)],
                            candidate_masses=[_v105_other_mass],
                            selected_cid=int(_contacted_cid),
                            event_type="read_other",
                        )
                    v915_buf.read_other_on_e3_contact(
                        ...
                        seed=int(seed),
                        visible_ratio_override=_v105_visible_ratio,
                    )
        ...
        window_rows.append({
            ...
            # v10.5 α-Integration (観察軸): 一旦 0 で予約、window 末で update
            "n_alphas_active": 0,
            "n_alphas_recorded": 0,
            "n_alphas_born": 0,
            "n_alphas_state_transitioned": 0,
            "max_alpha_size": 0,
            "mean_alpha_size": 0.0,
            "alpha_trigger_dist": "{}",
            # v10.5 β-Integration (会計単位): 一旦 0 で予約
            "n_betas_active": 0,
            "n_betas_recorded": 0,
            "n_betas_born": 0,
            "n_betas_state_transitioned": 0,
            "total_beta_q_inherited": 0,
            "total_beta_c_inherited": 0,
            "total_beta_q_distributed": 0,
            "total_beta_c_distributed": 0,
            "max_beta_alpha_size": 0,
            "mean_beta_alpha_size": 0.0,
            "max_beta_cid_size": 0,
            "mean_beta_cid_size": 0.0,
        })
        ...
        # v10.4: ghost 化直後に Integration へ Q/C 継承 (最強結合 1 つに全部)。
        for lid in (prev_lids - curr_lids):
            cid = cog.cid_for_lid(lid)
            # ledger から Q_remaining 読み取り (未登録 cid は 0)
            _residual_q = (v914_ledger.get_q_remaining(cid)
                           if cid is not None else 0)
            # v10.4: ghost 化前の C を読み取り (detach 後も cog.C は reap まで保持)
            _c_at_ghost = int(cog.C.get(cid, 0)) if cid is not None else 0
            cog.detach(lid, w,
                       residual_Q=_residual_q,
                       current_step=cumulative_step)
            # v10.4: Integration への継承 (cid が active な Integration に
            # 所属していれば、最強結合 1 つに Q/C を全継承)
            if cid is not None:
                v104_integration_mgr.on_ghost(
                    cid=cid,
                    q_at_ghost=_residual_q,
                    c_at_ghost=_c_at_ghost,
                    global_step=cumulative_step,
                    cog=cog, ledger=v914_ledger,
                )
        ...
        # v10.4: Integration の Q/C 再分配 (window 末、detach 後 reap 前)
        # active Integration の Q_inherited / C_inherited を active member_cids
        # の不足側に分配する (§6 状態依存逆張り分配)。
        # shadow_audit のときは log 記録のみ、実分配しない。
        v104_integration_mgr.redistribute_q_c(
            window=w, global_step=cumulative_step,
            cog=cog, ledger=v914_ledger,
            shadow_audit=be3_shadow_audit,
        )
        ...
        # v10.4 Integration: window summary を per_window 行に書き込み
        _v104_summary = v104_integration_mgr.get_window_summary()
        for _k, _v in _v104_summary.items():
            window_rows[-1][_k] = _v
        ...
        # v10.2: c_trajectory snapshot (hosted cid のみ、per cid × per window)
        # window 内の cognition / consciousness / skip 回数を per-cid で集計
        _bd_slice_for_window = v914_ledger.balance_decisions[
            _balance_count_at_window_start:]
        _per_cid_in_window = {}  # observer_cid -> {cog, con, skip}
        for _bd in _bd_slice_for_window:
            _ocid = _bd["observer_cid"]
            _stats = _per_cid_in_window.setdefault(
                _ocid, {"cog": 0, "con": 0, "skip": 0})
            _d = _bd["decision"]
            if _d == "cognition":
                _stats["cog"] += 1
            elif _d == "consciousness":
                _stats["con"] += 1
            else:
                _stats["skip"] += 1
        for _cid_h, _lid_h in cog.current_lid.items():
            if _lid_h is None:
                continue
            _le = v914_ledger.ledger.get(_cid_h)
            if _le is None:
                continue
            _stats = _per_cid_in_window.get(
                _cid_h, {"cog": 0, "con": 0, "skip": 0})
            v102_c_trajectory_rows.append({
                "seed": seed,
                "cid": int(_cid_h),
                "window": int(w),
                "step_at_window_end": int(cumulative_step),
                "C_at_window_end": int(cog.C.get(_cid_h, 0)),
                "Q_remaining_at_window_end": int(_le["v14_q_remaining"]),
                "n_cognition_in_window": int(_stats["cog"]),
                "n_consciousness_in_window": int(_stats["con"]),
                "n_skip_in_window": int(_stats["skip"]),
            })
    ...
    # v10.2 Probabilistic Balance: per_cid balance 統計
    cid_n_cognition_decisions = Counter()
    cid_n_consciousness_decisions = Counter()
    cid_n_balance_skipped = Counter()
    for bd in v914_ledger.balance_decisions:
        observer = bd["observer_cid"]
        d = bd["decision"]
        if d == "cognition":
            cid_n_cognition_decisions[observer] += 1
        elif d == "consciousness":
            cid_n_consciousness_decisions[observer] += 1
        else:  # "skip"
            cid_n_balance_skipped[observer] += 1
    ...
    for cid, meta in subject_meta.items():
        ...
        subject_rows.append({
            ...
            "C_at_run_end": (
                int(cog.C.get(cid, 0))
                if final_state == "hosted" else 0),
            "n_cognition_decisions":
                int(cid_n_cognition_decisions.get(cid, 0)),
            "n_consciousness_decisions":
                int(cid_n_consciousness_decisions.get(cid, 0)),
            "n_balance_skipped":
                int(cid_n_balance_skipped.get(cid, 0)),
            # ──────────────────────────────────────────────
            ...
            **build_v918_subject_columns(v915_self_buffers, cid),
            # ──────────────────────────────────────────────
            # v10.5 α-Integration (観察軸): per cid 集計
            # ──────────────────────────────────────────────
            "n_alphas_joined":
                int(v105_integration_mgr.alpha.cid_n_alphas_joined.get(cid, 0)),
            "n_alphas_currently":
                int(len(v105_integration_mgr.alpha.cid_to_alphas.get(cid, ()))),
            # ──────────────────────────────────────────────
            # v10.5 β-Integration (会計単位): per cid 集計
            # ──────────────────────────────────────────────
            "n_betas_joined":
                int(v105_integration_mgr.beta.cid_n_betas_joined.get(cid, 0)),
            "current_beta_id":
                (int(v105_integration_mgr.beta.cid_to_beta[cid])
                 if cid in v105_integration_mgr.beta.cid_to_beta else -1),
            "q_received_from_beta":
                int(v105_integration_mgr.beta.cid_q_received_from_betas.get(cid, 0)),
            "c_received_from_beta":
                int(v105_integration_mgr.beta.cid_c_received_from_betas.get(cid, 0)),
            "q_inherited_to_beta":
                int(v105_integration_mgr.beta.cid_q_inherited_to_beta.get(cid, 0)),
            "c_inherited_to_beta":
                int(v105_integration_mgr.beta.cid_c_inherited_to_beta.get(cid, 0)),
            # ──────────────────────────────────────────────
            # v10.5 Salience: per cid 集計
            # ──────────────────────────────────────────────
            "n_observed_as_target":
                int(v105_salience_tracker.cid_n_observed_as_target.get(cid, 0)),
            "n_selected_as_target":
                int(v105_salience_tracker.cid_n_selected_as_target.get(cid, 0)),
            "total_observed_mass":
                round(float(v105_salience_tracker.cid_total_observed_mass.get(cid, 0.0)), 4),
            # ──────────────────────────────────────────────
            # v10.5 Leakage: per cid 集計
            # ──────────────────────────────────────────────
            "total_leakage_received":
                int(v105_leakage_tracker.cid_total_leakage_received.get(cid, 0)),
        })
    ...
    # ════════════════════════════════════════════════════════
    # v10.2 SAVE: Probabilistic Balance logs (balance/)
    # ════════════════════════════════════════════════════════
    balance_dir = outdir / "balance"
    balance_dir.mkdir(parents=True, exist_ok=True)

    # ── balance_decisions_seed{N}.csv (raw、確率決定マスター) ──
    bd_path = balance_dir / f"balance_decisions_seed{seed}.csv"
    bd_fieldnames = [
        "seed", "window", "step", "global_step",
        "observer_cid", "contacted_cid",
        "is_ghost", "is_phantom",
        "residual_Q_at_decision",
        "Q_at_decision", "C_at_decision",
        "cognition_candidate", "consciousness_candidate",
        "P_cognition", "decision", "action_taken",
        "q_remaining_after", "c_after",
    ]
    with open(bd_path, "w", newline="") as f:
        ...
        for bd in v914_ledger.balance_decisions:
            row = {"seed": seed}
            for k in bd_fieldnames[1:]:
                row[k] = bd.get(k, "")
            writer.writerow(row)

    # ── c_trajectory_seed{N}.csv (per cid × per window) ──
    ct_path = balance_dir / f"c_trajectory_seed{seed}.csv"
    ct_fieldnames = [
        "seed", "cid", "window", "step_at_window_end",
    ...
    with open(ct_path, "w", newline="") as f:
        ...
        writer.writeheader()
        writer.writerows(v102_c_trajectory_rows)

    # ── balance_summary_seed{N}.csv (run-level) ──
    n_total_bd = len(v914_ledger.balance_decisions)
    n_cog_won = sum(1 for bd in v914_ledger.balance_decisions
                    if bd["decision"] == "cognition")
    n_con_won = sum(1 for bd in v914_ledger.balance_decisions
                    if bd["decision"] == "consciousness")
    n_skip_total = sum(1 for bd in v914_ledger.balance_decisions
                       if bd["decision"] == "skip")
    n_skip_q_zero_only = sum(
        1 for bd in v914_ledger.balance_decisions
        if (bd["decision"] == "skip"
            and not bd["cognition_candidate"]
            and bd["consciousness_candidate"]
            and bd["C_at_decision"] <= 0))
    n_skip_c_zero_only = sum(
        1 for bd in v914_ledger.balance_decisions
        if (bd["decision"] == "skip"
            and not bd["cognition_candidate"]
            and not bd["consciousness_candidate"]
            and bd["Q_at_decision"] <= 0
            and bd["C_at_decision"] > 0))
    n_skip_both_zero = sum(
        1 for bd in v914_ledger.balance_decisions
        if (bd["decision"] == "skip"
            and bd["Q_at_decision"] <= 0
            and bd["C_at_decision"] <= 0))
    n_skip_no_candidates = n_skip_total - (
        n_skip_q_zero_only + n_skip_c_zero_only + n_skip_both_zero)

    # 終端 C 分布 (hosted のみ)
    final_C_values = sorted(
        int(v) for cid_, v in cog.C.items()
        if cog.current_lid.get(cid_) is not None)
    ...
    C_max = max(final_C_values) if final_C_values else 0
    C_mean = (sum(final_C_values) / len(final_C_values)
              if final_C_values else 0.0)

    # CID 集団の Q+C 総和 (hosted のみ)
    Q_plus_C_total = 0
    for cid_, _lid in cog.current_lid.items():
        if _lid is None:
            continue
        ledger_entry = v914_ledger.ledger.get(cid_)
        if ledger_entry is None:
            continue
        Q_plus_C_total += int(
            ledger_entry["v14_q_remaining"]) + int(cog.C.get(cid_, 0))

    # ghost 集団の residual_Q 総和 (run 末の残り)
    ghost_residual_total = sum(
        int(v) for v in cog.ghost_residual_Q.values())

    # 散逸量 (主題 §3.1 G の数理通り)
    # CID 集団 Q+C 観点:
    #   E1/E2 spend: ΔQ=-1 → Δ(Q+C)=-1 (純散逸)
    #   認知 E3:    ΔQ=-1 + ΔC=+1 → Δ(Q+C)=0 (内部保存)
    #   意識 E3:    ΔQ=+received + ΔC=-1 → Δ(Q+C)=received-1 (流入)
    n_e1_e2 = sum(
        1 for ev in v914_ledger.events
        if (ev["v14_event_type"] in ("E1_death", "E1_birth",
                                       "E2_rise", "E2_fall")
            and ev["v14_spend_flag"]))
    total_received_via_consciousness = sum(
        int(e["received"]) for e in v914_ledger.ingestion_events)
    # 全系 (CID + ghost) Q+C 観点では digested も散逸:
    #   ghost.residual_Q -= digested → 系外消失
    total_digestion_dissipation = sum(
        int(e["digested"]) for e in v914_ledger.ingestion_events)

    bs_path = balance_dir / f"balance_summary_seed{seed}.csv"
    bs_fields = [
        "seed", "total_decisions",
        "n_cognition_won", "n_consciousness_won",
        "n_skip_q_zero_only", "n_skip_c_zero_only",
        "n_skip_both_zero", "n_skip_no_candidates",
        "C_max", "C_mean_at_run_end",
        "C_p25", "C_p50", "C_p75", "C_p95",
        "n_hosted_at_run_end",
        "Q_plus_C_total_at_run_end",
        "ghost_residual_Q_total_at_run_end",
        "n_e1_e2_spend", "total_received_via_consciousness",
        "total_digestion_dissipation",
    ]
    with open(bs_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=bs_fields)
        writer.writeheader()
        writer.writerow({
            "seed": seed,
            "total_decisions": n_total_bd,
            "n_cognition_won": n_cog_won,
            "n_consciousness_won": n_con_won,
            "n_skip_q_zero_only": n_skip_q_zero_only,
            "n_skip_c_zero_only": n_skip_c_zero_only,
            "n_skip_both_zero": n_skip_both_zero,
            "n_skip_no_candidates": n_skip_no_candidates,
            "C_max": C_max,
            "C_mean_at_run_end": round(C_mean, 4),
            "C_p25": _quantile(final_C_values, 0.25),
            "C_p50": _quantile(final_C_values, 0.50),
            "C_p75": _quantile(final_C_values, 0.75),
            "C_p95": _quantile(final_C_values, 0.95),
            "n_hosted_at_run_end": len(final_C_values),
            "Q_plus_C_total_at_run_end": Q_plus_C_total,
            "ghost_residual_Q_total_at_run_end": ghost_residual_total,
            "n_e1_e2_spend": n_e1_e2,
    ...
    # ════════════════════════════════════════════════════════
    # v10.5 SAVE: Integration logs (integration/) — α + β 並列
    # ════════════════════════════════════════════════════════
    integration_dir = outdir / "integration"
    integration_dir.mkdir(parents=True, exist_ok=True)

    # ─── α-Integration logs ──────────────────────────────────
    al_path = integration_dir / f"alpha_lifecycle_log_seed{seed}.csv"
    al_fields = [
        "seed", "step", "alpha_id", "event_type", "trigger_type",
        "member_cids",
    ]
    with open(al_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=al_fields)
        writer.writeheader()
        for r in v105_integration_mgr.alpha.lifecycle_log:
            writer.writerow({k: r.get(k, "") for k in al_fields})
    print(f"  v10.5 α: lifecycle_log: "
          f"{len(v105_integration_mgr.alpha.lifecycle_log)} rows -> {al_path}")

    am_path = integration_dir / f"alpha_membership_log_seed{seed}.csv"
    am_fields = ["seed", "step", "cid_id",
                 "alpha_ids", "binding_strengths"]
    _alpha_membership_rows = v105_integration_mgr.alpha.get_membership_snapshot_rows(
        cumulative_step)
    with open(am_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=am_fields)
        writer.writeheader()
        for r in _alpha_membership_rows:
            writer.writerow({k: r.get(k, "") for k in am_fields})
    print(f"  v10.5 α: membership_log: "
          f"{len(_alpha_membership_rows)} rows -> {am_path}")

    # ─── β-Integration logs ──────────────────────────────────
    bl_path = integration_dir / f"beta_lifecycle_log_seed{seed}.csv"
    bl_fields = [
        "seed", "step", "beta_id", "event_type",
        "member_alphas", "member_cids",
        "q_inherited_total", "c_inherited_total",
        "q_inherited_delta", "c_inherited_delta",
    ]
    with open(bl_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=bl_fields)
        writer.writeheader()
        for r in v105_integration_mgr.beta.lifecycle_log:
            writer.writerow({k: r.get(k, "") for k in bl_fields})
    print(f"  v10.5 β: lifecycle_log: "
          f"{len(v105_integration_mgr.beta.lifecycle_log)} rows -> {bl_path}")

    bd_path = integration_dir / f"beta_distribution_log_seed{seed}.csv"
    bd_fields = [
        "seed", "step", "beta_id", "target_cid",
        "q_distributed", "c_distributed",
        "target_q_ratio_before", "target_q_ratio_after",
    ]
    with open(bd_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=bd_fields)
        writer.writeheader()
        for r in v105_integration_mgr.beta.distribution_log:
            writer.writerow({k: r.get(k, "") for k in bd_fields})
    print(f"  v10.5 β: distribution_log: "
          f"{len(v105_integration_mgr.beta.distribution_log)} rows -> {bd_path}")

    bm_path = integration_dir / f"beta_membership_log_seed{seed}.csv"
    bm_fields = [
        "seed", "step", "beta_id", "state",
        "birth_step", "became_recorded_step",
        "n_member_alphas_active", "n_member_alphas_history",
        "n_member_cids_active", "n_member_cids_history",
        "member_alphas", "member_cids",
        "Q_inherited", "C_inherited",
    ]
    _beta_membership_rows = v105_integration_mgr.beta.get_membership_snapshot_rows(
        cumulative_step)
    with open(bm_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=bm_fields)
        writer.writeheader()
        for r in _beta_membership_rows:
            writer.writerow({k: r.get(k, "") for k in bm_fields})
    print(f"  v10.5 β: membership_log: "
          f"{len(_beta_membership_rows)} rows -> {bm_path}")

    # ─── Integration summary (α + β) ─────────────────────────
    is_path = integration_dir / f"integration_summary_seed{seed}.csv"
    # α 集計
    n_alpha_active = sum(
        1 for a in v105_integration_mgr.alpha.alphas.values()
        if a.state == "active")
    n_alpha_recorded = sum(
        1 for a in v105_integration_mgr.alpha.alphas.values()
        if a.state == "recorded")
    n_alpha_total = len(v105_integration_mgr.alpha.alphas)
    alpha_trigger_dist = Counter()
    for a in v105_integration_mgr.alpha.alphas.values():
        alpha_trigger_dist[a.trigger_type] += 1
    alpha_sizes_active = [
        len(a.member_cids)
        for a in v105_integration_mgr.alpha.alphas.values()
        if a.state == "active"]
    # β 集計
    n_beta_active = sum(
        1 for b in v105_integration_mgr.beta.betas.values()
        if b.state == "active")
    n_beta_recorded = sum(
        1 for b in v105_integration_mgr.beta.betas.values()
        if b.state == "recorded")
    n_beta_total = len(v105_integration_mgr.beta.betas)
    beta_alpha_sizes = [
        len(b.member_alphas)
        for b in v105_integration_mgr.beta.betas.values()
        if b.state == "active"]
    beta_cid_sizes = [
        len(b.member_cids)
        for b in v105_integration_mgr.beta.betas.values()
        if b.state == "active"]
    total_q_inh_beta = sum(
        v105_integration_mgr.beta.cid_q_inherited_to_beta.values())
    total_c_inh_beta = sum(
        v105_integration_mgr.beta.cid_c_inherited_to_beta.values())
    total_q_dist_beta = sum(
        v105_integration_mgr.beta.cid_q_received_from_betas.values())
    total_c_dist_beta = sum(
        v105_integration_mgr.beta.cid_c_received_from_betas.values())

    is_fields = [
        "seed",
        # α
        "n_alpha_total", "n_alpha_active", "n_alpha_recorded",
        "n_birth_be3", "n_birth_open_triad",
        "n_birth_closed_triad", "n_birth_third_overlap",
        "max_alpha_active_size", "mean_alpha_active_size",
        # β
        "n_beta_total", "n_beta_active", "n_beta_recorded",
        "max_beta_alpha_size", "mean_beta_alpha_size",
        "max_beta_cid_size", "mean_beta_cid_size",
        "total_beta_q_inherited_run", "total_beta_c_inherited_run",
        "total_beta_q_distributed_run", "total_beta_c_distributed_run",
    ]
    with open(is_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=is_fields)
        writer.writeheader()
        writer.writerow({
            "seed": seed,
            "n_alpha_total": n_alpha_total,
            "n_alpha_active": n_alpha_active,
            "n_alpha_recorded": n_alpha_recorded,
            "n_birth_be3": alpha_trigger_dist.get("be3", 0),
            "n_birth_open_triad": alpha_trigger_dist.get("open_triad", 0),
            "n_birth_closed_triad": alpha_trigger_dist.get("closed_triad", 0),
            "n_birth_third_overlap":
                alpha_trigger_dist.get("third_overlap", 0),
            "max_alpha_active_size":
                max(alpha_sizes_active) if alpha_sizes_active else 0,
            "mean_alpha_active_size":
                round(sum(alpha_sizes_active) / len(alpha_sizes_active), 4)
                if alpha_sizes_active else 0.0,
            "n_beta_total": n_beta_total,
            "n_beta_active": n_beta_active,
            "n_beta_recorded": n_beta_recorded,
            "max_beta_alpha_size":
                max(beta_alpha_sizes) if beta_alpha_sizes else 0,
            "mean_beta_alpha_size":
                round(sum(beta_alpha_sizes) / len(beta_alpha_sizes), 4)
                if beta_alpha_sizes else 0.0,
            "max_beta_cid_size":
                max(beta_cid_sizes) if beta_cid_sizes else 0,
            "mean_beta_cid_size":
                round(sum(beta_cid_sizes) / len(beta_cid_sizes), 4)
                if beta_cid_sizes else 0.0,
            "total_beta_q_inherited_run": int(total_q_inh_beta),
            "total_beta_c_inherited_run": int(total_c_inh_beta),
            "total_beta_q_distributed_run": int(total_q_dist_beta),
            "total_beta_c_distributed_run": int(total_c_dist_beta),
        })
    print(f"  v10.5 Integration summary: α(total={n_alpha_total} "
          f"active={n_alpha_active} recorded={n_alpha_recorded}) "
          f"β(total={n_beta_total} active={n_beta_active} "
          f"recorded={n_beta_recorded}) "
          f"alpha_trigger_dist={dict(alpha_trigger_dist)}")

    # ════════════════════════════════════════════════════════
    # v10.5 SAVE: Salience event log (salience/)
    # ════════════════════════════════════════════════════════
    salience_dir = outdir / "salience"
    salience_dir.mkdir(parents=True, exist_ok=True)
    sa_path = salience_dir / f"salience_event_log_seed{seed}.csv"
    sa_fields = [
        "seed", "step", "observer_cid", "candidate_cid",
        "candidate_mass", "selected", "event_type",
    ]
    with open(sa_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=sa_fields)
        writer.writeheader()
        for r in v105_salience_tracker.event_log:
            writer.writerow({k: r.get(k, "") for k in sa_fields})
    print(f"  v10.5 Salience: event_log: "
          f"{len(v105_salience_tracker.event_log)} rows -> {sa_path}")

    # ════════════════════════════════════════════════════════
    # v10.5 SAVE: Leakage event log (leakage/)
    # ════════════════════════════════════════════════════════
    leakage_dir = outdir / "leakage"
    leakage_dir.mkdir(parents=True, exist_ok=True)
    lk_path = leakage_dir / f"leakage_event_log_seed{seed}.csv"
    lk_fields = [
        "seed", "step", "recipient_cid", "source_cid",
        "recorded_beta_id", "leakage_amount", "trigger_event",
    ]
    with open(lk_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=lk_fields)
        writer.writeheader()
        for r in v105_leakage_tracker.event_log:
            writer.writerow({k: r.get(k, "") for k in lk_fields})
    print(f"  v10.5 Leakage: event_log: "
          f"{len(v105_leakage_tracker.event_log)} rows -> {lk_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--be3-shadow-audit", action="store_true",
        help="v10.3 双方向 E3 を log のみ記録 (C 消費なし、shadow audit モード)。"
    )
    ...
    run(seed=args.seed,
        ...
        be3_shadow_audit=args.be3_shadow_audit,
        N=args.N)
