...
class SpendAuditLedger:
    ...
    def __init__(self, delta_fn=None, disable_e3: bool = False,
                 cog=None, ingestion_rng=None) -> None:
        ...
        # observe_step 中の E3 onset で (hosted_a, ghost_b, link_id) を集めて、
        # 同じ observe_step 末尾で 1 CID あたり 1 ghost を選定して摂食する。
    ...
    def observe_step(self, *, window: int, step: int, global_step: int,
        ...
        # v10.1 Minimal Ingestion: ingestion 候補の収集
        #   E3 emission ループの最中に hosted_a ↔ ghost_b ペアを集める。
        #   ghost 側は cid_ctx に居ない (hosted only) ので、observer ループでは
        #   skip されるが、cog で is_ghost(b) が真なら ingestion 候補として保存。
        ingest_candidates_per_observer: dict = {}  # observer_cid -> [(ghost_cid, link_id)]
        ...
        for (cid_a, cid_b, lk) in new_e3_pairs:
            ...
            for observer_cid, contacted_cid in (
                ...
                # v10.1: contacted_cid が ghost (active) または reaped なら
                # ingestion 候補。reaped は phantom contact として記録される
                # (認知層では消滅したが、_node_to_cids の物理痕跡経由で
                # E3 onset が発火したケース)。
        ...
        # v10.1 Ingestion phase:
        #   1. observer (hosted_a) の cid_id 昇順
        #   2. observer ごとに seeded RNG で 1 ghost をランダム選定
        #   3. 確定ペアを順次 attempt_ingestion
        #   step 内では reap せず、cog.reap_ghosts_step は orchestrator が
        #   per-step ループ末尾で呼ぶ。
