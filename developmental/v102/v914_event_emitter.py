...
def detect_e3_new_pairs(alive_l_set: set, node_to_cids: dict,
    ...
    v10.2 修正 (§I 事前齟齬指摘): 戻り値を (c1, c2, lk) で明示 sort する。
    CPython の set/dict イテレーション順は整数タプルなら決定論的だが、
    v10.2 では balance_rng の draw 順が pair 処理順に依存するため、
    Python マイナーバージョン変更でも安定するよう保険として明示 sort。
    """
    new_pairs = []
    ...
    # v10.2: 明示 sort (cid_a, cid_b, link) の辞書順 (§I)
