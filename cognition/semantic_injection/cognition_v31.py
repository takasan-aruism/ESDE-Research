#!/usr/bin/env python3
"""
ESDE Cognition v3.1 — Concept Dynamics
========================================
Phase : Cognition (v3.1)
Role  : Claude (Implementation)
Arch  : Gemini | Audit: GPT

PURPOSE
-------
Statistical validation of concept island persistence across 100 seeds.
Topological tracking of Merge/Split events between concept islands.
Correlation between observer divergence and semantic entropy.

CHANGES FROM v3.0
------------------
  1. Seed batch expanded to 100.
  2. Merge detection: cross-boundary link weight exceeds threshold + phase
     alignment (cos(mean Δθ) > 0.5) between A-dominated and B-dominated islands.
  3. Split detection: a concept island present in window W-1 fragments into
     2+ disconnected components in window W.
  4. Per-region semantic entropy for divergence-entropy correlation.
  5. Pearson correlation computed per run across windows.

NO PHYSICS CHANGES. Injection mechanism identical to v3.0.
...
-----
  ...
  # Sanity
  python cognition_v31.py --sanity

  # Full 100 seeds (tmux recommended)
  parallel -j 40 python cognition_v31.py --seed {1} \
    ::: $(python -c "print(' '.join(str(s) for s in range(1,101)))")

  # Aggregate
  python cognition_v31.py --aggregate
"""
...
# ================================================================
# COGNITION v3.1 CONSTANTS
# ================================================================
COG_N           = 5000
COG_PLB         = 0.007
COG_RATE        = 0.002
COG_SEEDS       = list(range(1, 101))  # 100 seeds
N_CONCEPTS_DEF  = 2
PHASE_SPREAD    = 0.3
CONCEPT_NAMES   = {0: "A", 1: "B", 2: "C"}

# Merge/Split thresholds (topological, not semantic)
MERGE_WEIGHT_THR    = 5.0   # total cross-boundary link weight between two islands
MERGE_PHASE_THR     = 0.5   # cos(mean phase diff) > this → phase-aligned
SPLIT_MIN_COMP_SIZE = 3     # minimum fragment size to count as a split product
...
OUTPUT_DIR = Path("outputs_v31")


# ================================================================
# CONCEPT CLUSTER ASSIGNMENT (identical to v3.0)
# ================================================================
...
# ================================================================
# CONCEPT WINDOW METRICS (extended from v3.0)
# ================================================================
def compute_concept_window(state, concept_map, n_concepts,
    ...
    Returns v3.0 metrics + per-island concept labels for merge/split tracking.
    """
    ...
    # v3.1: labeled islands for merge/split
    labeled_islands = []  # list of (concept_id, frozenset_of_nodes)
    ...
    # --- 3. Concept entropy among C-nodes (global) ---
    ...
    # --- 3b. Per-region semantic entropy (v3.1) ---
    region_entropy = {}
...
# ================================================================
# MERGE / SPLIT DETECTION (v3.1)
# ================================================================
def detect_merge_split(state, concept_map, n_concepts,
                       prev_labeled, curr_labeled):
    """
    Detect merge and split events between consecutive windows.

    Merge: two islands of different concepts share enough cross-boundary
           link weight AND are phase-aligned.
    Split: an island from prev_window has nodes now in 2+ disconnected
           components in curr_window.

    Returns: (merge_events, split_events) — lists of dicts.
    """
    merges = []
    splits = []

    # --- Merge detection ---
    # For each pair of differently-labeled current islands,
    # check cross-boundary link weight and phase alignment.
    for i, (c_i, nodes_i) in enumerate(curr_labeled):
        for j, (c_j, nodes_j) in enumerate(curr_labeled):
            if j <= i or c_i == c_j:
                continue
            # Cross-boundary links between the two islands
            cross_weight = 0.0
            cross_count = 0
            phase_diffs = []
            for n in nodes_i:
    ...
    # --- Split detection ---
    # For each island in prev_window, check if its nodes are now
    # fragmented into multiple disconnected components.
    if prev_labeled:
        # Build current adjacency for alive strong links
        curr_adj = defaultdict(set)
        for (a, b) in state.alive_l:
            if state.S[(a, b)] >= 0.20:  # mid-threshold
                curr_adj[a].add(b)
                curr_adj[b].add(a)

        for prev_c, prev_nodes in prev_labeled:
            # Which of prev_nodes are still alive?
            alive_prev = frozenset(n for n in prev_nodes if n in state.alive_n)
            if len(alive_prev) < SPLIT_MIN_COMP_SIZE * 2:
                continue

            # Find connected components among alive_prev using current edges
            visited = set()
            components = []
            for n in alive_prev:
                if n in visited:
...
def aggregate():
    ...
    n = len(results)
    print(f"\n{'='*80}")
    print(f"  ESDE Cognition v3.1 — Aggregate Summary ({n} seeds)")
    print(f"{'='*80}")
    ...
    for cn in ["A", "B"]:
        ...
        print(f"    {cn}: persist={np.mean(persis):.3f}±{np.std(persis):.3f} "
              f"max_streak={np.mean(streaks):.1f}±{np.std(streaks):.1f} "
              f"mean_islands={np.mean(islands):.2f}±{np.std(islands):.2f}")

    # Boundary
    print(f"\n  Boundary Friction:")
    print(f"    Mean links: {np.mean([r['mean_boundary_links'] for r in results]):.1f}")
    print(f"    Mean weight: {np.mean([r['mean_boundary_weight'] for r in results]):.4f}")
    print(f"    Mean entropy: {np.mean([r['mean_concept_entropy'] for r in results]):.4f}")

    # Merge/Split
    print(f"\n  Merge/Split Events:")
    mc = [r["merge_count"] for r in results]
    sc = [r["split_count"] for r in results]
    print(f"    Merges: mean={np.mean(mc):.2f} median={np.median(mc):.0f} "
          f"max={max(mc)} seeds_with_merge={sum(1 for x in mc if x > 0)}/{n}")
    print(f"    Splits: mean={np.mean(sc):.2f} median={np.median(sc):.0f} "
          f"max={max(sc)} seeds_with_split={sum(1 for x in sc if x > 0)}/{n}")
    ...
    if valid_corr:
        print(f"    Mean r: {np.mean(valid_corr):.4f}±{np.std(valid_corr):.4f} "
              f"(n={len(valid_corr)} valid)")
        pos = sum(1 for c in valid_corr if c > 0)
        neg = sum(1 for c in valid_corr if c < 0)
        print(f"    Direction: positive={pos} negative={neg}")
    else:
    ...
    csv_path = OUTPUT_DIR / "cognition_v31_summary.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="ESDE Cognition v3.1 — Concept Dynamics")
    ...
    if args.sanity:
        ...
        res, wlogs, clogs, sw, ml, sl = run_cognition(
        ...
        for cn in ["A", "B"]:
            if cn in res["concept_summary"]:
                ...
                print(f"  Concept {cn}: persist={cs['persistence_frac']:.2f} "
                      f"streak={cs['max_streak']} isl={cs['mean_islands']:.2f}")
        ...
        print(f"  Corr(div,ent)={res['corr_divergence_entropy']}")
    ...
    for seed in seeds:
        ...
        print(f"  seed={seed} (N={args.N}, concepts={args.n_concepts})...",
              flush=True)
        result, wlogs, clogs, sw, ml, sl = run_cognition(
        ...
        print(f"    k*={result['global_dominant_k']} "
              ...
              f"corr={result['corr_divergence_entropy']} "
