#!/usr/bin/env python3
"""
ESDE Cognition v3.3 — Concept Attraction
==========================================
Phase : Cognition (v3.3)
Role  : Claude (Implementation)
Arch  : Gemini | Audit: GPT

PURPOSE
-------
Observe whether interacting concept islands form stable cross-island
bridges and whether diffusion flow is symmetric or asymmetric.

v3.2 confirmed: ~27k diffusion events per run, merge=0, split=0,
entropy stable. Concepts exchange influence without collapsing.

v3.3 adds OBSERVATION ONLY — no new mechanisms:
  1. Bridge tracking: cross-concept links that persist across windows
  2. Diffusion flow asymmetry: A→B vs B→A event counts

Physics, diffusion, and seeding are identical to v3.2.

USAGE
-----
  cd ~/esde/ESDE-Research/cognition/semantic_injection

  python cognition_v33.py --sanity
  parallel -j 20 python cognition_v33.py --seed {1} ::: $(seq 1 20)
  python cognition_v33.py --aggregate
"""
...
# ================================================================
# CONSTANTS (v3.2 params preserved)
# ================================================================
COG_N           = 5000
COG_PLB         = 0.007
COG_RATE        = 0.002
COG_SEEDS_SMALL = list(range(1, 21))
COG_SEEDS_FULL  = list(range(1, 101))
N_CONCEPTS_DEF  = 2
PHASE_SPREAD    = 0.3
...
# v3.3: bridge persistence threshold
BRIDGE_S_THR        = 0.15  # link strength to count as a bridge
...
OUTPUT_DIR = Path("outputs_v33")


# ================================================================
# CONCEPT ASSIGNMENT (unchanged)
# ================================================================
...
def concept_zone_summary(concept_map, N):
    ...
    return ", ".join(
        f"{CONCEPT_NAMES.get(c, 'neutral') if c >= 0 else 'neutral'}={counts[c]}"
...
# ================================================================
# SEMANTIC DIFFUSION (v3.2, now returns direction counts)
# ================================================================
def apply_semantic_diffusion(state, concept_map, concept_centers,
                             boundary_nodes, rng):
    """
    v3.3: returns (total_events, flow_dict) where flow_dict maps
    (source_concept, target_concept) -> count.
    """
    events = 0
    flow = Counter()  # (from_c, to_c) -> count
...
# ================================================================
# BOOSTED SEEDING (v3.2, unchanged)
# ================================================================
...
# ================================================================
# CONCEPT WINDOW METRICS (v3.1/v3.2, unchanged)
# ================================================================
...
# ================================================================
# v3.3: BRIDGE TRACKING
# ================================================================
def compute_bridges(state, concept_map, prev_bridges):
    """
    Cross-concept links with S >= BRIDGE_S_THR are candidate bridges.
    A bridge that appeared in the previous window AND this window is
    "persistent". Returns: (current_bridge_set, new_count, persistent_count,
    total_weight, mean_lifespan_update).
    prev_bridges: dict {link_key: lifespan_in_windows}
    """
    curr = {}
    for (i, j) in state.alive_l:
...
# ================================================================
# MERGE / SPLIT DETECTION (v3.1/v3.2, unchanged)
# ================================================================
...
def run_cognition(seed, N=COG_N, plb=COG_PLB, rate=COG_RATE,
    ...
    total_flow = Counter()  # v3.3: cumulative (from_c, to_c) counts
    ...
    # v3.3: bridge state
    prev_bridges = {}  # {link_key: lifespan}
    ...
    for step in range(quiet_steps):
        ...
        # === v3.2: DIFFUSION (v3.3: with flow tracking) ===
        ...
        if (step + 1) % WINDOW == 0:
            ...
            clog["bridge_weight"] = bridge_stats["total_weight"]
    ...
    if len(concept_window_logs) >= 5:
        ...
        for wl, cl in zip(window_logs, concept_window_logs):
            ...
            ent_series.append(np.mean([cl.get(f"r{r}_entropy", 0)
                                       for r in range(N_REGIONS)]))
    ...
    # v3.3: flow asymmetry
    flow_a_to_b = total_flow.get((0, 1), 0)
    flow_b_to_a = total_flow.get((1, 0), 0)
    flow_total = flow_a_to_b + flow_b_to_a
    flow_asymmetry = round((flow_a_to_b - flow_b_to_a) / max(flow_total, 1), 4)
...
def aggregate():
    ...
    print(f"\n  Diffusion Flow:")
    ab = [r["flow_A_to_B"] for r in results]
    ba = [r["flow_B_to_A"] for r in results]
    asym = [r["flow_asymmetry"] for r in results]
    print(f"    A→B: {np.mean(ab):.0f}  B→A: {np.mean(ba):.0f}")
    print(f"    Asymmetry: {np.mean(asym):.4f}±{np.std(asym):.4f} "
          f"(+ve = A→B dominant)")

    print(f"\n  Bridges:")
    nb = [r["mean_n_bridges"] for r in results]
    ...
    print(f"    Mean count: {np.mean(nb):.1f}  "
          f"Persistent: {np.mean(pb):.1f}")
    print(f"    Lifespan: mean={np.mean(ml):.2f}  "
          f"max={max(mx) if mx else 0}")

    mc = [r["merge_count"] for r in results]
    sc = [r["split_count"] for r in results]
    print(f"\n  Merge={sum(mc)} total ({sum(1 for x in mc if x>0)}/{n} seeds)  "
          f"Split={sum(sc)} total ({sum(1 for x in sc if x>0)}/{n} seeds)")
    ...
    for r in results:
        row = {
            ...
            "diffusion": r["total_diffusion_events"],
            "flow_AB": r["flow_A_to_B"], "flow_BA": r["flow_B_to_A"],
            "flow_asym": r["flow_asymmetry"],
    ...
    csv_path = OUTPUT_DIR / "cognition_v33_summary.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="ESDE Cognition v3.3 — Concept Attraction")
    ...
    if args.sanity:
        ...
        print(f"  Diffusion: {res['total_diffusion_events']}  "
              f"A→B={res['flow_A_to_B']} B→A={res['flow_B_to_A']} "
              f"asym={res['flow_asymmetry']:.4f}")
    ...
    for seed in seeds:
        ...
        print(f"    k*={result['global_dominant_k']} "
              f"bridges={result['mean_n_bridges']:.0f} "
              f"persist={result['mean_persistent_bridges']:.0f} "
              f"life={result['mean_bridge_lifespan']:.1f} "
              f"A→B={result['flow_A_to_B']} B→A={result['flow_B_to_A']} "
