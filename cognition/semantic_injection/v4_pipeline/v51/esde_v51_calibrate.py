#!/usr/bin/env python3
"""
ESDE v5.1 — Closed Circulation Calibration
=============================================
USAGE
-----
  # Sanity
  python esde_v51_calibrate.py --seed 42 --windows 10

  # Standard
  parallel -j 2 python esde_v51_calibrate.py --seed {1} --windows 200 ::: 42 123

  # Ablation: circulation off (v4.9 behavior + RNG injection)
  python esde_v51_calibrate.py --seed 42 --windows 100 --no-circulation

  # Aggregate
  python esde_v51_calibrate.py --aggregate --output calibration_v51
"""
...
from esde_v51_engine import V51Engine, V51EncapsulationParams, V51_WINDOW
...
LOG_FIELDS = [
    ...
    # v5.1 §1 V→E
    "birth_v_consumed_tot", "birth_e_excitation_tot",
    # v5.1 §3 E→V
    "boil_v_added_tot", "boil_to_birth_ratio",
    # v5.1 §4 Phase torque
    "prox_phase_torque_events", "mean_phase_torque",
]
...
def run(seed, n_windows, window_steps, output_dir, encap_params):
    ...
    print(f"  history={encap_params.history_enabled} "
          ...
          f"keep_bg={encap_params.keep_bg}")
    print(f"  Injection...", flush=True)

    engine = V51Engine(seed=seed, encap_params=encap_params)
    ...
    bg_tag = "_keepbg" if encap_params.keep_bg else ""
    tag = f"v51_seed{seed}_{h_tag}_{v_tag}_{c_tag}{bg_tag}"
    ...
    for w in range(n_windows):
        ...
        torque_events = cc.get("prox_phase_torque_events", 0)
        total_torque = cc.get("total_phase_torque", 0)
        mean_torque = round(total_torque / max(1, torque_events), 6)
        ...
        row = {
            ...
            # v5.1 §1 V→E
            "birth_v_consumed_tot": round(
                cc.get("birth_v_consumed_tot", 0), 4),
            "birth_e_excitation_tot": round(
                cc.get("birth_e_excitation_tot", 0), 4),
            # v5.1 §3 E→V
            "boil_v_added_tot": round(
                cc.get("boil_v_added_tot", 0), 4),
            "boil_to_birth_ratio": cc.get("boil_to_birth_ratio", 0),
            # v5.1 §4 Phase torque
            "prox_phase_torque_events": torque_events,
            "mean_phase_torque": mean_torque,
        }
        writer.writerow(row)
        f.flush()
        ...
        print(f"  {frame.window:>4} {isum['n_clusters']:>4} "
              ...
              f"{cc.get('boil_v_added_tot',0):>5.0f} "
              f"{cc.get('birth_e_excitation_tot',0):>5.1f} "
              f"{torque_events:>4} "
    ...
    print(f"  Π={engine.current_pi:.4f} "
          f"heat={cc.get('heat_total',0):.4f} "
          f"boil={cc.get('boil_v_added_tot',0):.1f} "
          f"excite={cc.get('birth_e_excitation_tot',0):.4f}")
...
def aggregate(output_dir):
    output_dir = Path(output_dir)
    json_files = sorted(output_dir.glob("v51_*_detail.json"))
    if not json_files:
        print(f"  No v5.1 JSON files in {output_dir}")
        return
    print(f"\n  {'='*80}")
    print(f"  ESDE v5.1 — Aggregate ({len(json_files)} runs)")
    print(f"  {'='*80}")
...
def main():
    parser = argparse.ArgumentParser(
        description="ESDE v5.1 Closed Circulation")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--windows", type=int, default=200)
    parser.add_argument("--window-steps", type=int, default=V51_WINDOW)
    parser.add_argument("--output", type=str, default="calibration_v51")
    ...
    parser.add_argument("--keep-bg", action="store_true",
                        help="Ablation: keep canonical bg seeding (§2 off)")
