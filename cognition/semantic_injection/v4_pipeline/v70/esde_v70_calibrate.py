#!/usr/bin/env python3
"""
ESDE v7.0 — World Induction Calibration
==========================================
USAGE
-----
  python esde_v70_calibrate.py --seed 42 --windows 10
  parallel -j 2 python esde_v70_calibrate.py --seed {1} --windows 200 ::: 42 123
  python esde_v70_calibrate.py --seed 42 --windows 50 --no-recurrence  # v6.0 baseline
  python esde_v70_calibrate.py --aggregate --output calibration_v70
"""
...
from esde_v70_engine import V70Engine, V70EncapsulationParams, V70_WINDOW
...
def run(seed, n_windows, window_steps, output_dir, encap_params):
    ...
    engine = V70Engine(seed=seed, encap_params=encap_params)
    engine.run_injection()

    r_tag = "recur" if encap_params.recurrence_enabled else "norecur"
    tag = f"v70_seed{seed}_{r_tag}"
    ...
    for w in range(n_windows):
        ...
        print(f"  {frame.window:>4} {isum['n_clusters']:>4} "
              ...
              f"{vl.get('recurrence_entries',0):>4} "
              f"{vl.get('motifs_detected',0):>3} "
    ...
    print(f"  Virtual: energy={vlsum['virtual_energy_total']:.2f} "
          f"recurrence={vlsum['recurrence_entries']} "
          f"labels={vlsum['labels_active']}")
    for lb in vlsum.get("label_details", []):
...
def aggregate(output_dir):
    output_dir = Path(output_dir)
    json_files = sorted(output_dir.glob("v70_*_detail.json"))
    if not json_files:
        print(f"  No v7.0 JSON files in {output_dir}")
        return
    print(f"\n  {'='*80}")
    print(f"  ESDE v7.0 — Aggregate ({len(json_files)} runs)")
    print(f"  {'='*80}")
    for jf in json_files:
        with open(jf) as fh:
            d = json.load(fh)
        m = d["meta"]
        vl = d.get("virtual_layer_summary", {})
        print(f"  seed={m['seed']} links={m['final_alive_links']} "
              f"labels={vl.get('labels_active',0)} "
              f"energy={vl.get('virtual_energy_total',0):.1f} "
              f"M={m['max_milestone']}")
    print()
...
def main():
    parser = argparse.ArgumentParser(
        description="ESDE v7.0 World Induction")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--windows", type=int, default=200)
    parser.add_argument("--window-steps", type=int, default=V70_WINDOW)
    parser.add_argument("--output", type=str, default="calibration_v70")
