...
==========================================
v0.1: bonus_mode parameter — "triangle", "cycle", or "none"
  "cycle" mode: decay bonus from ALL cycles <= max_cycle_length
  Weighted: shorter cycles = stronger bonus
"""
...
class PhysicsEngine:
    ...
    def _apply_dissipation(self, universe):
        ...
        for node in universe.nodes.values():
            if node.alive:
                ...
                eff = p.node_decay_rate / (1.0 + p.connectivity_shelter * lsum)
                node.energy *= (1.0 - eff)

        for key, lk in universe.links.items():
            if not lk.alive:
                continue
            base = p.link_decay_rate
            if p.structural_advantage_enabled:
                bonus = self._structure_bonus(universe, lk.i, lk.j)
                eff = base / (1.0 + bonus)
            else:
                eff = base
            lk.strength *= (1.0 - eff)

    def _structure_bonus(self, universe, li, lj):
        p = self.params
        if p.bonus_mode == "none" or not p.structural_advantage_enabled:
            return 0.0
        if p.bonus_mode == "triangle":
            return p.gamma * universe.triangle_count_for_link(li, lj)
        if p.bonus_mode == "cycle":
            cc = universe.cycle_count_for_link(li, lj, p.max_cycle_length)
            ws = 0.0
            for k in range(3, p.max_cycle_length + 1):
                ws += p.cycle_weights.get(k, 0.0) * cc.get(k, 0)
            return p.gamma * ws
        return 0.0
    ...
    def inject(self, universe, target_nodes=None):
        ...
        for nid in target_nodes:
            universe.nodes[nid].energy = min(1.0, universe.nodes[nid].energy + p.inject_amount)
    ...
    def _apply_exclusion(self, universe):
        ...
        for node in universe.nodes.values():
            ...
            if total <= node.c_max:
                continue
            conn = [(k, l) for k, l in universe.links.items()
                    if l.alive and (l.i == node.id or l.j == node.id)]
            conn.sort(key=lambda x: x[1].birth_step, reverse=True)
            cur = total
            for k, l in conn:
                if cur <= node.c_max:
                    break
    ...
    def _apply_energy_flow(self, universe):
        ...
        for key, lk in universe.links.items():
            if not lk.alive:
                continue
            ei = universe.nodes[lk.i].energy
            ej = universe.nodes[lk.j].energy
            f = k * lk.strength * (ei - ej)
            flows[lk.i] -= f
            flows[lk.j] += f
